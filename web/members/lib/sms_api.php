...
class SmsInterface {
	...
	function getCreditsRemaining ()
	{
        ...
        // invalid resposne
        if ($this->responseCode != 120 && $this->responseCode != 100)
            return -2;

        // Parse the credits.
        // String of the form "1[20]0 OK( [0-9]+ credits remaining)".
        $cred = substr ($s, 7);
        $idx = (strpos ($cred, ' '));

        if ($idx == false){
		    if ($this->responseCode == 100)
                // postpaid accounts return status 100 (not 120), and for negative
                // credit limits the gateway returns nothing, so we return 0
                return 0;
            else
                return -2;
        }
 ...
 *
 * PLEASE NOTE: the internal structure of this class will definitely change
 * with the rollout of the MessageMedia web serivces.
 *
 * LIMITATIONS:
 *  1. Requires PHP 5 and CURL
 *  2. Does not support proxy or SSL connections
 */
class SmsAccountManagement {
    private $username;
    private $password;
    ...
    /**
     * Constructor
     * @param   username    The username of the administrator user used to manage the account
     * @param   password    The password associated with the above username, in plain text
     */
     ...
     * @param   newUser             Return param. The username of the newly created user
     * @param   newPassword         Return param. The password (in plaintext) of the newly created user
     * @param   creditLimitType     When the credit limit should be reset. May be: daily, monthly, never (default daily)
     *
    ...
    public function createNewUser (
        &$newUser,
        &$newPassword,
        $creditLimitType = "daily"
    ){
        // make a connection to the TEMPORARY webservice
        // this should be replaced by the real XML webservice shortly. ECLS 31/05/10
        $this->callWebservice ("createUser", $ret);

        // parse the output for the username/password or error message
        $lines = explode("\n", $ret);
        if (sizeof($lines) < 2)
            return false;
        ...
        // if for some reason the username or password is blank, consider this a failure
        // but don't unset username or password; it may be useful for diagnostic purposes
        if (strlen ($newUser) == 0 || strlen ($newPassword) == 0)
            return false;

        if ($creditLimitType != "daily"){
            $this->setUserCreditLimit ($newUser, NULL, NULL, $creditLimitType);
        }
    ...
    public function setUserCreditLimit (
        ...
        $this->setCurlPostData ("targetUsername", $username, true);

        if ($frequency !== NULL && ($frequency == "daily"
                || $frequency == "monthly" || $frequency == "never")){
            $this->setCurlPostData ("targetValue", $frequency);
            if (!$this->callWebservice ("setCreditLimitFrequency"))
                return false;
        }

        if ($currentLimit !== NULL){
            $this->setCurlPostData ("targetValue", $currentLimit);
            if (!$this->callWebservice ("setCreditLimit"))
                return false;
        }

        if ($nextLimit !== NULL){
            $this->setCurlPostData ("targetValue", $nextLimit);
            if (!$this->callWebservice ("setNextCreditLimit"))
                return false;
        }
     ...
     *                          Not returned for daily credit limit users
     *
    ...
    public function getUserCreditLimit (
        ...
        if (!$this->callWebservice ("getCreditLimitDetails", $ret))
            return false;
    ...
    /**
     * Set the post data that will be submitted with the curl request
     * @param   key     POST data name
     * @param   key     POST data value
     * @param   reset   if TRUE, reset the post data to the default values
     *                  before adding the key/value pair, if set to FALSE the
     *                  key/value pair will be added to the current data
     */
    private function setCurlPostData (
        ...
        if (empty($this->curlPostData)){
            $this->curlPostData["username"] = $this->username;
            $this->curlPostData["password"] = $this->password;
        }
    ...
    /**
     * Call the webservice. Requires curl
     *
     * @param   action  The action to perform via the web service
     * @param   data    Any additional data that should be passed through to the webservice
     * @param   result  The output from the webservice in plaintext. Only set if the request succeeded
     *
    ...
    private function callWebservice (
        ...
        // temporary webservices placeholder URL
        $url = "http://access.m4u.com.au/webservicesPlaceHolder.php";

        // set the action
        $this->setCurlPostData ("action", $action);
        ...
        if ($res === FALSE){
            ...
            // The response is from the URL, but was an error
            $this->error = $res;
            return false;
        } elseif (substr(curl_getinfo($ch, CURLINFO_HTTP_CODE),0,1) == 4
                || substr(curl_getinfo($ch, CURLINFO_HTTP_CODE),0,1) == 5){
            // There was an error returned from the URL
            $this->error = "Request returned error code " . curl_getinfo($ch, CURLINFO_HTTP_CODE);
            return false;
        }
