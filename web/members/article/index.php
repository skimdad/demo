<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /members/article/index.php
	# ----------------------------------------------------------------------------------------------------
	...
	$pageObj = new pageBrowsing("Article", $screen, RESULTS_PER_PAGE, (($_GET["newest"])?("id DESC"):("level DESC, title")), "title", $letter, $where);
	$articles = $pageObj->retrievePage();

	$paging_url = DEFAULT_URL."/members/".ARTICLE_FEATURE_FOLDER."/index.php";
...
?>
			...
			<div class="content">
				...
				<h2><?=system_showText(LANG_MENU_MANAGEARTICLE);?></h2>
				...
				<? if ($articles) { ?>
										...
										<tr>
											<td width="50%">
												<table align="center" border="0" cellpadding="0" cellspacing="0" class="warningBOX">
													<tr>
														<th><a href="<?=DEFAULT_URL?>/members/billing/index.php" class="warningBOXcontent"><?=system_showText(LANG_MSG_CONTINUE_TO_PAY_ARTICLE)?></a></th>
					...
					<? } else { ?>
						<? include(INCLUDES_DIR."/tables/table_paging.php"); ?>
						<p class="informationMessage"><?=system_showText(LANG_NO_ARTICLES_IN_THE_SYSTEM)?></p>
					<? } ?>
