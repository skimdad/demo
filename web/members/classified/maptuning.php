<?
	...
	if ($id) {
		...
		if ((!$classified->getString("address")) && (!$classified->getString("zip_code")) && (!$classified->getNumber("location_3")) && (!$classified->getNumber("location_4"))) {
			header("Location: ".DEFAULT_URL."/members/".CLASSIFIED_FEATURE_FOLDER."/index.php?screen=$screen&letter=$letter");
			exit;
		}
		$itemObj = $classified;
...
?>
	...
	<div class="content">
		...
		<ul class="list-view">
			<li class="list-back"><a href="<?=DEFAULT_URL?>/members/<?=CLASSIFIED_FEATURE_FOLDER;?>/<?=($search_page) ? "search.php" : "index.php"?>?screen=<?=$screen?>&letter=<?=$letter?>"><?=system_showText(LANG_LABEL_BACK);?></a></li>
		</ul>
		...
		<form id="formclassifiedmaptuningcancel" action="<?=DEFAULT_URL?>/members/<?=CLASSIFIED_FEATURE_FOLDER;?>/index.php" method="post">
