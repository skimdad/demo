<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /members/event/maptuning.php
	# ----------------------------------------------------------------------------------------------------
	...
	if ($id) {
		...
		if ((!$event->getString("address")) && (!$event->getString("zip_code")) && (!$event->getNumber("location_3")) && (!$event->getNumber("location_4"))) {
			header("Location: ".DEFAULT_URL."/members/".EVENT_FEATURE_FOLDER."/index.php?screen=$screen&letter=$letter");
			exit;
		}
		$itemObj = $event;
...
?>
	...
	<div class="content">
		...
		<ul class="list-view">
			<li class="list-back"><a href="<?=DEFAULT_URL?>/members/<?=EVENT_FEATURE_FOLDER;?>/<?=($search_page) ? "search.php" : "index.php"?>?screen=<?=$screen?>&letter=<?=$letter?>"><?=system_showText(LANG_LABEL_BACK);?></a></li>
		</ul>
		...
		<form name="maptuning_form" id="maptuning_form" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
            ...
            <div class="baseButtons">
                ...
                <p class="standardButton">
                    <button type="button" value="Cancel" onclick="document.getElementById('formeventmaptuningcancel').submit();"><?=system_showText(LANG_BUTTON_CANCEL)?></button>
                </p>
            </div>     

		</form>
		<form id="formeventmaptuningcancel" action="<?=DEFAULT_URL?>/members/<?=EVENT_FEATURE_FOLDER;?>/index.php" method="post">
