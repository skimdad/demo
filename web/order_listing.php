...
/* var x=0;
if (document.getElementById("chk_deal_" + get_level + "_1")) x = x + Number((document.getElementById("chk_deal_" + get_level + "_1").value));
if (document.getElementById("chk_deal_" + get_level + "_2")) x = x + Number((document.getElementById("chk_deal_" + get_level + "_2").value));
if (document.getElementById("chk_deal_" + get_level + "_3")) x = x + Number((document.getElementById("chk_deal_" + get_level + "_3").value));
if (document.getElementById("chk_deal_" + get_level + "_4")) x = x + Number((document.getElementById("chk_deal_" + get_level + "_4").value));
if (document.getElementById("chk_deal_" + get_level + "_5")) x = x + Number((document.getElementById("chk_deal_" + get_level + "_5").value));
alert("chk_deal_" + get_level + "_1");
if (x>0) alert(x); */
				
				
				xmlhttp.open("GET", "<?=DEFAULT_URL;?>/ordercalculateprice.php?item=listing&item_id=<?=$unique_id;?>&level="+get_level+"&categories="+get_categories+"&listingtemplate_id="+get_listingtemplate_id+"&discount_id="+get_discount_id, true);
				xmlhttp.send(null);
			}
		}
		...
		function levelSwitch(level) {
			...
			foreach ($levelValue as $value) {
				...
				$freedeals = '<table class="standard-table"  style="width: 300px; font-size:14px;color: #666666;font-family: Arial,Helvetica,Sans-serif;">';
				for ($i = 1; $i <= ($listingLevelObj->getFreeDeal($value)); $i++){
					$freedeals .= "<tr><td>Deal-".($i)." (Included)</td><td align=\"right\">FREE</td></tr>";
				}
				$freedealsamount = 0;
				for ($i = $listingLevelObj->getFreeDeal($value)+1; $i <= 5; $i++){
					$freedealsamount += $listingLevelObj->getExtraDealPrice($value);
					$freedealschk = '<input id="chk_deal_'.$value.'_'.$i.'" name="chk_deal_'.$value.'_'.$i.'" style="margin-top:5px;margin-left:-10px;" type="checkbox" checked="checked" value="'.$listingLevelObj->getExtraDealPrice($value).'">Include';
					$freedeals .= "<tr><td>Deal-".($i)."</td><td align=\"right\">".CURRENCY_SYMBOL.$listingLevelObj->getExtraDealPrice($value)."</td><td align=\"right\">".$freedealschk."</td></tr>";
				}
				$freedeals .= "<tr style=\"font-size:14px;font-weight:bold;\"><td>Deals Total</td><td  align=\"right\">".CURRENCY_SYMBOL.number_format($freedealsamount,2)."</td></tr>";
				$freedeals .= "</table>";
				echo "var freedeal_note_".$value." = '".$freedeals."';";
			}
			?>
			document.order_listing.level.value = level;
			if (document.getElementById("extracategory_note")) {
			       document.getElementById("extracategory_note").innerHTML = eval("extracat_note_" + level);
				   document.getElementById("freedeal_note").innerHTML = eval("freedeal_note_" + level);
			}
			orderCalculate();
			if (document.formDirectory != undefined)	document.formDirectory.action = "<?=$formloginaction?>&query=level=" + document.order_listing.level.value + "&listingtemplate_id=" + document.order_listing.listingtemplate_id.value;
			if (document.formOpenID != undefined)		document.formOpenID.action = "<?=$formloginaction?>&query=level=" + document.order_listing.level.value + "&listingtemplate_id=" + document.order_listing.listingtemplate_id.value;
			...
			<div id="formNewUser" class="<? if ((!$usertype) || ($usertype == "newuser")) echo "isVisible"; else echo "isHidden"; ?>">
				...
				<form name="order_listing" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post" class="standardForm" onsubmit="JS_submit();">
					...
					<div id="purchase_promotion" style="display: <?php echo ($listingLevelObj->getHasPromotion($level) == 'y') ? '' : 'none' ?>;">
						...
						<!--<span id="freedeal_note" style="float:left; width:400px;"><?=$freedeals?></span> -->															
					</div>
