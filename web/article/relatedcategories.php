<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /article/relatedcategories.php
	# ----------------------------------------------------------------------------------------------------
	...
	if ($keyword) {
		...
		if (ARTICLECATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			$sql = "SELECT * FROM ArticleCategory WHERE category_id = 0 AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND enabled = 'y' AND ".$where_clause."";
		} else {
			$sql = "SELECT * FROM ArticleCategory WHERE lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND enabled = 'y' AND ".$where_clause."";
		}
		...
		while ($row = mysql_fetch_assoc($rs)) {
			$related_categories[] = new ArticleCategory($row["id"]);
		}
	...
	if ($related_categories) {
		...
		$category_tree = system_generateCategoryTree($related_categories, $arr_full_path, "article", $user, EDIR_LANGUAGE);
	}
