<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /conf/realestate_scheme.inc.php
	# ----------------------------------------------------------------------------------------------------
	...
	//set default scheme
	$edir_default_scheme = "realestate";

	//set all available schemes separated by comma
	$edir_schemes = "realestate,custom";
	$edir_schemenames = "Real Estate,Custom";

	//code to setup one specific scheme from all available schemes
	$edir_scheme = "realestate";

	if (DEMO_LIVE_MODE == 0) {
		@include_once(EDIRECTORY_ROOT.'/custom/domain_'.SELECTED_DOMAIN_ID.'/theme/realestate_scheme.inc.php');
	} else {
