<?
	...
	if (defined("EDIRECTORY_ROOT")) {
		$handleMobile = fopen(EDIRECTORY_ROOT."/conf/loadconfig.inc.php", "r");
	} else {
		$handleMobile = fopen("../conf/loadconfig.inc.php", "r");
	}
	if ($handleMobile) {
		...
		do {
			...
			if (($posMobile = strpos($bufferMobile, $lookforMobile)) !== false) {
				...
				$edirectory_folder = substr($edirectory_folder, 0, strpos($edirectory_folder, "\""));
    ...
    if (strpos($_SERVER["HTTP_HOST"], EDIRECTORY_MOBILE_LABEL) !== false) {
		...
		if((EDIRECTORY_FOLDER)){
			...
			define("MOBILE_EDIRECTORY_FOLDER", "/".EDIRECTORY_MOBILE_LABEL);
		}
