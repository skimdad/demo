<?php
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST") && !$_POST["account_sugar_id"]) {
		...
		if ($boolean_seckey && $validate_account && $validate_contact && $validate_listing && $validate_discount) {
			...
			// the following code is added by Debi on 18th Aug., 2012
	...
	if ($categories) {
		...
		//Issue :: It was showing default length 1 for feed listbox(javascript)
		//Done by sagar mehetre
...
<a href="#" id="info_window" class="iframe fancy_window_categPath" style="display:none"></a>   
	...
	<script language="javascript" type="text/javascript">
		...
		function orderCalculate() {
			...
			if (xmlhttp) {
				...
				var get_level = 0;
				if (document.order_listing.level) 
					get_level = document.order_listing.level.value;
				var get_categories = 0;
				if (document.order_listing.feed) 
					get_categories = document.order_listing.feed.length;
		...
		function levelSwitch(level, hasPromotion) {
			...
			if (level != 70 && document.getElementById("check_out_payment")) {
				document.getElementById("check_out_payment").className = "isVisible";
				if (document.getElementById("check_out_free")) document.getElementById("check_out_free").className = "isHidden";
			} else {
				if (document.getElementById("check_out_free")) document.getElementById("check_out_free").className = "isVisible";
				if (document.getElementById("check_out_payment")) document.getElementById("check_out_payment").className = "isHidden";
			...
			foreach ($levelValue as $value) {
				...
				for ($i = 1; $i <= ($listingLevelObj->getFreeDeal($value)); $i++){
					$freedeals .= "<tr><td>Deal-".($i)." (Included)</td><td>FREE</td></tr>";
				}
				for ($i = $listingLevelObj->getFreeDeal($value)+1; $i <= 5; $i++){
					$freedealschk = '<input id="chk_deal_'.$value.'_'.$i.'" name="chk_deal_'.$value.'_'.$i.'" style="margin-top:5px;margin-left:-10px;" type="checkbox" checked="checked">Include';
					$freedeals .= "<tr><td>Deal-".($i)."</td><td>".CURRENCY_SYMBOL.$listingLevelObj->getExtraDealPrice($value)."</td><td>".$freedealschk."</td></tr>";
				}
	...
	<?
	$checkoutpayment_class = "isVisible";
...
<!-- Gap here....-->
					...
					<? if (PAYMENT_FEATURE == "on") { ?>
						...
						<table align="left" border="0" cellpadding="2" cellspacing="2" class="standardSIGNUPTable">
						  <tr>
									<th><?=string_ucwords(system_showText(LANG_LABEL_DISCOUNTCODE))?>:</th>
									<td><input type="text" name="discount_id" value="<?=$discount_id?>" class="input-form-listing" style="margin-top: -4px;" maxlength="10" onblur="orderCalculate();" /></td>
								</tr></table>
