<?
	...
	**************************************************************************************/
	...
	function db_getPromotionFromDBXML($table,$by_key="",$by_value="",$number=1,$orderby="",$fields, $sql=false, $domain_id = SELECTED_DOMAIN_ID, $where_featured = "", $where_calability = ""){
		...
		if(is_array($fields)){
			...
			while ($row = mysql_fetch_assoc($result)) {
				if ($row["total"] > 0) {
					...
					$left = $row["left"];
					$right = $row["right"];
					$sql2 = "SELECT * FROM ListingCategory WHERE $where_featured lang <> '' $where_calability AND root_id = $root_id AND `left` <= $left AND `right` >= $right";
			...
			if (is_array($catAux)) {
				foreach ($catAux as $key => $row) {
					${"title".$labelsuffix}[$key]  = $row["title".$labelsuffix];
				}
				array_multisort(
						${"title".$labelsuffix}, SORT_ASC,
						$catAux);
				...
				foreach ($catAux as $cat) {
					$string_XML .= "<item>";
					for($i=0;$i<count($fields);$i++){
						$string_XML .= "<".$fields[$i].">".format_getString($cat[$fields[$i]])."</".$fields[$i].">";
					}
					$string_XML .= "</item>";
				}
				$string_XML .= "</".$table.">";
			...
			if(count($xml_subcategories->item) > 0) {
				...
				for($j=0;$j<count($xml_subcategories->item);$j++){
					...
					if (count($subcategory > 0)) {						
						...
						if ($subcategory["title".$langIndex]) {
							$nameArray[] = $marker.$subcategory["title".$langIndex];
						} else{
							foreach ($arrLangs as $lang) {
								if ($langIndex != $lang) {
									if ($subcategory["title".$lang]){
										$nameArray[] = $marker.$subcategory["title".$lang];
										break;
									}
								}
							}
						...
						$subLevel = $level + 1;
						if ($subLevel <= CATEGORY_LEVEL_AMOUNT && $optimization != "on") {
