<?
	...
	if (is_string($categories)) { ?>
		...
		<h2>
			...
			<? if (LISTINGCATEGORY_SCALABILITY_OPTIMIZATION == "on") { ?>
				<a class="view-more" href="<?=LISTING_DEFAULT_URL?>/allcategories.php"><?=system_showText(LANG_LISTING_VIEWALLCATEGORIES)?></a>
			<? } ?>
		</h2>
			...
			for($i=0;$i<count($xml_categories->info);$i++){
				...
				if($categories){
					...
					if (MODREWRITE_FEATURE == "on") {
						$categoryLink = LISTING_DEFAULT_URL."/guide/".$categories["friendly_url".$langIndex];
					} else {
						$categoryLink = LISTING_DEFAULT_URL."/results.php?category_id=".$categories["id"];
					}
					...
					if(SHOW_CATEGORY_COUNT == "on"){
						 echo " <span>(".$categories["active_listing"].")</span>";
					}
								...
								if(count($xml_subcategories->info) > 0) {
									...
									for($j=0;$j<count($xml_subcategories->info);$j++){
										...
										if($subcategories){
											...
											if (MODREWRITE_FEATURE == "on") {
												$subCategoryLink = LISTING_DEFAULT_URL."/guide/".$categories["friendly_url".$langIndex]."/".$subcategories["friendly_url".$langIndex];
											} else {
												$subCategoryLink = LISTING_DEFAULT_URL."/results.php?category_id=" . $subcategories["id"];
											}
											...
											if (SHOW_CATEGORY_COUNT == "on") echo " <span>(".$subcategories["active_listing"].")</span>";
