<?
	...
	$edir_themes = "default,realestate";
	$edir_themenames = "eDirectory Default,Real Estate";

	//code to setup one specific theme from all available themes
	$edir_theme = "default";

    if(strpos($_SERVER["PHP_SELF"], "members")){
        @include_once(EDIRECTORY_ROOT."/custom/domain_".URL_DOMAIN_ID."/theme/theme.inc.php");
        $edir_themeMembers = $edir_theme;
    }
	...
	define("EDIR_THEME_MEMBERS",    $edir_themeMembers);

	unset($edir_default_theme);
	unset($edir_themes);
	unset($edir_themenames);
	unset($edir_theme);
	unset($edir_themeMembers);
