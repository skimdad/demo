<?
	...
	# ----------------------------------------------------------------------------------------------------
/*	//synergy
    // The zipcode and where do not work together. So put blank in where while using zip code
	if ($_GET['zip'] != ''){
		$temp_where = $_GET['where'];
		$_GET['where'] = '';
	}
else{	
$mystring = trim($_GET['where']);
$findme   = 'New York';
$pos = strpos($mystring, $findme);
	if ($pos === false) {
		...
		$temp_where = $_GET['where'];
		$_GET['where'] = str_ireplace($findme, 'ny', $_GET['where']);
	}
} */
	$search_lock = false;
//synergy
