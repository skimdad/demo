<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /sitemgr/export/eventexport.php
	# ----------------------------------------------------------------------------------------------------
	...
	if($aux_export_running["finished"] == "N" && EVENT_SCALABILITY_OPTIMIZATION == "on"){
	...
	<script language="javascript" type="text/javascript">
		...
		function startExportProcess() {
			...
			if (xmlhttp_startexportprocess) {
				xmlhttp_startexportprocess.open("GET", "./itemexportfile.php?export_type=event&file=<?=$exportFile?>&domain_id=<?=SELECTED_DOMAIN_ID?>", true);
		...
		function removeExportControl() {
			...
			if (xmlhttp_removeexportcontrol) {
				xmlhttp_removeexportcontrol.open("GET", "./itemexportfile.php?export_type=event&file=<?=$exportFile?>&removecontrol=true&domain_id=<?=SELECTED_DOMAIN_ID;?>", true);
		...
		function checkExportProgress() {
			...
			if (xmlhttp_checkexportprogress) {
						...
						if (xmlhttp_checkexportprogress.status == 200) {
							...
							if (isNaN(current_progress)) {
								...
								if (parseInt(document.getElementById("export_progress").innerHTML) >= 100) {
									...
									document.getElementById("export_message").innerHTML = "<?=system_showText(LANG_SITEMGR_EXPORT_EXPORTDONE).(EVENT_SCALABILITY_OPTIMIZATION == "on" ? " <br /><a href=".DEFAULT_URL."/sitemgr/export/exportfile.php?export_type=event&filename=".$exportFile."&type=csv>".system_showText(LANG_SITEMGR_EXPORT_LASTFILE_MESSAGE)."</a>" : "");?>";
									document.getElementById("export_progress").innerHTML = "&nbsp;";
									document.getElementById("export_progress_percentage").innerHTML = "&nbsp;";
									removeExportControl();
								} else {
				...
				xmlhttp_checkexportprogress.open("GET", "./itemexportcheck.php?export_type=event&file=<?=$exportFile?>&lastprogress="+lastprogress+"&domain_id=<?=SELECTED_DOMAIN_ID?>", true);
	...
	<div id="main-right">
		...
		<div id="content-content">
			...
			<div class="default-margin">
				...
				<p class="informationMessage"><?=system_showText(LANG_SITEMGR_EXPORT_EXPORTEVENT_TIP)?></p>
						...
						<span id="export_message">
							<a href="javascript:startExport();" class="standardLink" style="margin: 0; padding: 0; font-size: 15px;">
								...
								if(EVENT_SCALABILITY_OPTIMIZATION == "on"){
									if ($old_export_file){
									?>
									<a href="<?=DEFAULT_URL."/sitemgr/export/exportfile.php?export_type=event&filename=".$old_export_file."&type=csv"?>"><?=system_showText(LANG_SITEMGR_EXPORT_LASTFILE_MESSAGE)?></a>
									<?
									}
								}
								?>
