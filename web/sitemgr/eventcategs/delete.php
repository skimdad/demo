<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /sitemgr/eventcategs/delete.php
	# ----------------------------------------------------------------------------------------------------
	...
	if ($id) {
		...
		$category = db_getFromDB("eventcategory", "id", $id, 1, "", "object", SELECTED_DOMAIN_ID, false, $fields);

		$dbMain = db_getDBObject(DEFAULT_DB, true);
		$db = db_getDBObjectByDomainID(SELECTED_DOMAIN_ID, $dbMain);
		$sql = "SELECT COUNT(0) as row_amount FROM Event WHERE
		cat_1_id = '$id' 
		OR cat_2_id = '$id' 
		OR cat_3_id = '$id' 
		OR cat_4_id = '$id' 
        ...
        $message = 0;
		header("Location: ".DEFAULT_URL."/sitemgr/eventcategs/".(($search_page) ? "search.php" : "index.php")."?message=".$message."&category_id=".$category_id."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
		exit;
	}
