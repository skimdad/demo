	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if($listingdefaultprice) { 
			...
			if (validate_form("listingdefaultprice", $_POST, $message_listingdefaultprice)) {
				...
				foreach ($free_deal as $freeDealLevel=>$freeDealValue) {
                    $listingLevelObj->updatePricing("free_deal", $freeDealValue, $freeDealLevel);
				}

				foreach ($extra_deal_price as $extraDealPriceLevel=>$extraDealPriceValue) {
                    $listingLevelObj->updatePricing("extra_deal_price", $extraDealPriceValue, $extraDealPriceLevel);
				}				
				
				/* 				
				$promotionPriceObj = new PromotionPrice();
				echo "<pre>";
				var_dump($promotionPriceObj);
				echo "</pre>";
				...
				foreach ($extra_promotions as $extrapromotionslevel => $extrapromotionsValue) {
					$promotionPriceObj->updatePricing($extrapromotionslevel, $extrapromotionslevel);
				} */
...
<div id="main-right">
	...
	<div id="content-content">
		<div class="default-margin">
			...
			<? /* echo 'if (PROMOTION_FEATURE == "on" && CUSTOM_PROMOTION_FEATURE == "on") { ?>
