<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /sitemgr/locations/location_5/delete.php
	# ----------------------------------------------------------------------------------------------------
	...
	$url_redirect = "".DEFAULT_URL."/sitemgr/locations/location_5";
	...
	# ----------------------------------------------------------------------------------------------------
	if ($id) {
		$location = new Location5($id);
	} else {
        header("Location: ".DEFAULT_URL."/sitemgr/locations/location_5/index.php?".($_location_node_params?$_location_node_params."&":"")."message=".$message."&screen=$screen&letter=$letter");
        exit;
	}
        ...
        <div id="content-content">
            <div class="default-margin">
                ...
                <form name="location" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
                    ...
                    <p class="informationMessage"><?=system_showText(LANG_SITEMGR_LOCATION_DELETEQUESTION)?> <?=system_showText(constant("LANG_SITEMGR_LABEL_".LOCATION5_SYSTEM));?> <?=$location->getString("name")?> ?</p>

                    <button type="submit" value="Submit" class="input-button-form"><?=system_showText(LANG_SITEMGR_SUBMIT)?></button>
                    <button type="button" value="Cancel" class="input-button-form" onclick="document.getElementById('formlocation5deletecancel').submit();"><?=system_showText(LANG_SITEMGR_CANCEL)?></button>
                </form>
                <form id="formlocation5deletecancel" action="<?=DEFAULT_URL?>/sitemgr/locations/location_5/index.php<?=($_location_node_params?"?".$_location_node_params."&":"?")?>operation=cancel" method="post">
                    <?=system_getFormInputHiddenParams((($_POST)?($_POST):($_GET)));?>
                </form>
