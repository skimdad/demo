<?
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		if (($id) && ($event)) {
			...
			header("Location: ".DEFAULT_URL."/sitemgr/".EVENT_FEATURE_FOLDER."/event.php?level=".$_POST['level']);
                exit;
            }
        }
...
<div id="main-right">
	...
	<div id="content-content">
		<div class="default-margin">
			...
			<form id="formeventlevelcancel" action="<?=DEFAULT_URL?>/sitemgr/<?=EVENT_FEATURE_FOLDER;?>/<?=(($search_page) ? "search.php" : "index.php");?>" method="post">
				<input type="hidden" name="letter" value="<?=$letter?>" />
				<input type="hidden" name="screen" value="<?=$screen?>" />
				<?=system_getFormInputSearchParams((($_POST)?($_POST):($_GET)));?>
			</form>
