<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /sitemgr/listingcategs/featured.php
	# ----------------------------------------------------------------------------------------------------
	...
	$url_redirect = "".DEFAULT_URL."/sitemgr/listingcategs/featured.php";
    $url_base = "".DEFAULT_URL."/sitemgr";
    $sitemgr = 1;
    $featuredListingCategory = 1;
	...
	if (!isset($pop_categories)) system_setFreqActions('listing_featuredcateg','listing');
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['save'] == 1) {
		...
		if ($non_feat_categories!="")
			system_changeFeaturedAtribute ("ListingCategory", "$non_feat_categories", "n" );
		if ($feat_categories!="")
			system_changeFeaturedAtribute ("ListingCategory", "$feat_categories", "y" );
        $message = ucfirst(LANG_SITEMGR_FEATUREDLISTINGSCATEGORIES_WERESUCCESSUPDATED);
	...
	$pageObj   = new pageBrowsing("ListingCategory", $screen, RESULTS_PER_PAGE, "title, id", $letterfield, $letter, "category_id = ".db_formatNumber($category_id), $fields);
    $categories = $pageObj->retrievePage("array");

    $paging_url = DEFAULT_URL."/sitemgr/listingcategs/featured.php";
...
<div id="top-content">
	<div id="header-content">
		<h1><?=string_ucwords(system_showText(LANG_SITEMGR_LISTING_PLURAL))?> - <?=string_ucwords(system_showText(LANG_SITEMGR_FEATUREDCATEGORY_PLURAL))?> </h1>
	</div>
</div>
