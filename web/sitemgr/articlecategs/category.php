<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /sitemgr/articlecategs/category.php
	# ----------------------------------------------------------------------------------------------------
	...
	if (!isset($id)) system_setFreqActions('articlecateg_add','ARTICLE_FEATURE');
...
<div id="main-right">
	<div id="top-content">
		<div id="header-content">
			<h1><?=string_ucwords(system_showText(LANG_SITEMGR_ARTICLE_PLURAL))?> - <?=$_GET["id"] ? system_showText(LANG_SITEMGR_EDIT) : system_showText(LANG_SITEMGR_ADD) ;?> <?=string_ucwords(system_showText(LANG_SITEMGR_CATEGORY))?></h1>
		</div>
	</div>
	<div id="content-content">
		<div class="default-margin">
			...
			<form name="category" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
				...
				<? if ($id) { $subcategories = db_getFromDB("articlecategory", "category_id", $id, "all", "", "object", SELECTED_DOMAIN_ID);} ?>
			...
			<form action="<?=DEFAULT_URL?>/sitemgr/articlecategs/<?=(($search_page) ? "search.php" : "index.php");?>" method="get">
