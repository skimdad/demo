<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /sitemgr/articlecategs/featured.php
	# ----------------------------------------------------------------------------------------------------
	...
	$url_redirect = "".DEFAULT_URL."/sitemgr/articlecategs/featured.php";
    $url_base = "".DEFAULT_URL."/sitemgr";
    $sitemgr = 1;
    $featuredArticleCategory = 1;
	...
	if (!isset($pop_categories)) system_setFreqActions('article_featuredcateg','ARTICLE_FEATURE');
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['save'] == 1) {
		...
		if ($non_feat_categories!="")
			system_changeFeaturedAtribute ("ArticleCategory", "$non_feat_categories", "n" );
		if ($feat_categories!="")
			system_changeFeaturedAtribute ("ArticleCategory", "$feat_categories", "y" );
        $message = ucfirst(LANG_SITEMGR_FEATUREDARTICLESCATEGORIES_WERESUCCESSUPDATED);
    }
    ...
    $paging_url = DEFAULT_URL."/sitemgr/articlecategs/featured.php";
...
<div id="top-content">
	<div id="header-content">
		<h1><?=string_ucwords(system_showText(LANG_SITEMGR_ARTICLE_PLURAL))?> - <?=string_ucwords(system_showText(LANG_SITEMGR_FEATUREDCATEGORY_PLURAL))?> </h1>
	</div>
</div>
