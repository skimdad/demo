<?
	...
	if (!in_array($level, $evLevelValue)) {
		header("Location: ".DEFAULT_URL."/advertise.php?event");
		exit;
	}
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($boolean_seckey && $validate_account && $validate_contact && $validate_event && $validate_discount) {
			...
			if ($emailNotificationObj = system_checkEmail(SYSTEM_EVENT_SIGNUP, $contact->getString("lang"))) {
				...
				<html>
					...
					<body>
						<div class=\"email_style_settings\">
							...
							$sitemgr_msg .= "Event:<br /><br />";
							$sitemgr_msg .= "<b>Title: </b>".$event->getString("title")."<br />";
							$sitemgr_msg .= "<br /><a href=\"".DEFAULT_URL."/sitemgr/".EVENT_FEATURE_FOLDER."/view.php?id=".$event->getNumber("id")."\" target=\"_blank\">".DEFAULT_URL."/sitemgr/".EVENT_FEATURE_FOLDER."/view.php?id=".$event->getNumber("id")."</a><br /><br />
						</div>
					</body>
				</html>";
			...
			if ($sitemgr_event_emails[0]) {
				foreach ($sitemgr_event_emails as $sitemgr_event_email) {
					system_mail($sitemgr_event_email, "[".EDIRECTORY_TITLE."] Signup Notification", $sitemgr_msg, EDIRECTORY_TITLE." <$sitemgr_event_email>", "text/html", '', '', $error);
				}
			}
			}
			////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if ($payment_method == "checkout") {
				if ($hasPackage){
					header("Location: ".DEFAULT_URL."/members/signup/packages.php?item_type=event&item_level=".$level."&checkout=true&payment_method=checkout&item_id=".$event->getNumber("id"));
				} else {
					header("Location: ".DEFAULT_URL."/members/".EVENT_FEATURE_FOLDER."/event.php?id=".$event->getNumber("id")."&process=signup");
				}
			} elseif ($payment_method == "invoice") {
				if ($hasPackage){
					header("Location: ".DEFAULT_URL."/members/signup/packages.php?item_type=event&item_level=".$level."&checkout=false&payment_method=invoice&item_id=".$event->getNumber("id"));
				} else {
					header("Location: ".DEFAULT_URL."/members/signup/invoice.php");
				}
			} else {
				if ($hasPackage){
					header("Location: ".DEFAULT_URL."/members/signup/packages.php?item_type=event&item_level=".$level."&checkout=false&payment_method=".$payment_method."&item_id=".$event->getNumber("id"));
				} else {
	...
	$formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/members/login.php?destiny=".EDIRECTORY_FOLDER.(EDIR_LANG_URL ? "/".EDIR_LANGUAGEABBREVIATION : "")."/members/".EVENT_FEATURE_FOLDER."/eventlevel.php";
    ...
    <script language="javascript" type="text/javascript">
		...
		function orderCalculate() {
			...
			if (xmlhttp) {
				...
				var get_level = 0;
				if (document.order_event.level) get_level = document.order_event.level.value;
				var get_discount_id = "";
				if (document.order_event.discount_id) get_discount_id = document.order_event.discount_id.value;
				xmlhttp.open("GET", "<?=DEFAULT_URL;?>/ordercalculateprice.php?item=event&item_id=<?=$unique_id;?>&level="+get_level+"&discount_id="+get_discount_id, true);
				xmlhttp.send(null);
			}
		}
		...
		function levelSwitch(level) {
			document.order_event.level.value = level;
			orderCalculate();
	...
	<div class="content content-full">
		...
		<div class="content-main">
					...
					<td>
						<table align="center" cellspacing="2" cellpadding="2" border="0" class="standardChooseLevel">
							<? foreach ($levelValue as $value) { ?>
								<tr>
									<th><?=$eventLevelObj->showLevel($value)?></th>
									<td>
										<?
										if ($eventLevelObj->getPrice($value) > 0) {
											echo CURRENCY_SYMBOL.$eventLevelObj->getPrice($value)." ".system_showText(LANG_PER)." ";
											if (payment_getRenewalCycle("event") > 1) {
												echo payment_getRenewalCycle("event")." ";
												echo payment_getRenewalUnitNamePlural("event");
											}else {
								...
								<tr>
									<td colspan="2">
										...
										<div id="formFacebookUser" class="<? if ($usertype == "facebookuser") echo "isVisible"; else echo "isHidden"; ?>">
											...
											<? $urlRedirect = "?advertise=yes&advertise_item=event&item_id=".$unique_id."&destiny=".urlencode(DEFAULT_URL."/".MEMBERS_ALIAS."/".EVENT_FEATURE_FOLDER."/eventlevel.php"); ?>
			...
			<div id="formNewUser" class="<? if ((!$usertype) || ($usertype == "newuser")) echo "isVisible"; else echo "isHidden"; ?>">
				...
				<? if ($message_account || $message_event || $message_discount || $message_contact) { ?>
					<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardMessage">
						...
						<? } ?>
						<? if ($message_event || $message_discount) { ?>
							<tr>
								<th colspan="2" class="SIGNUPTable-title errorTitle"><?=system_showText(LANG_EVENTINFO_ERROR)?></th>
							</tr>
							<? if ($message_event) { ?>
								<tr>
									<td colspan="2"><p class="errorMessage"><?=$message_event?></p></td>
								</tr>
							<? } ?>
							<? if ($message_discount) { ?>
								<tr>
				...
				<form name="order_event" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post" class="standardForm">
					<input type="hidden" name="advertise" value="yes" />
					...
					<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardSIGNUPTable">
						...
						<tr>
							<th colspan="2" class="SIGNUPTable-title"><h2 class="standardSubTitle"><?=system_showText(LANG_EVENTINFO)?></h2></th>
						</tr>
						<tr>
							<th>* <?=system_showText(LANG_LABEL_TITLE)?>:</th>
							<td>
								<input type="text" name="title" value="<?=$title?>" class="input-form-event" maxlength="100" onblur="easyFriendlyUrl(this.value, 'friendly_url', '<?=FRIENDLYURL_VALIDCHARS?>', '<?=FRIENDLYURL_SEPARATOR?>');" />
						...
						<? if (PAYMENT_FEATURE == "on") { ?>
							<? if ((CREDITCARDPAYMENT_FEATURE == "on") || (INVOICEPAYMENT_FEATURE == "on")) { ?>
								<tr>
									...
									<td><input type="text" name="discount_id" value="<?=$discount_id?>" class="input-form-event" maxlength="10" onblur="orderCalculate();" /></td>
								</tr>
							<? } ?>
						<? } ?>
					</table>
					<table id="range_date" align="center" border="0" cellpadding="2" cellspacing="2" class="standardSIGNUPTable">
		
		
						<tr>
							<th>* <?=system_showText(LANG_LABEL_STARTDATE)?>:</th>
							<td>
								<table class="tableOrderEvent_date" border="0" cellpadding="0" cellspacing="0">
									<tr>
										<td class="eventDate"><input type="text" name="start_date" id="start_date" value="<?=$start_date?>" /> <span>(<?=format_printDateStandard()?>)</span></td>
									</tr>
								</table>
							</td>
						</tr>
						...
						<tr>
							<th>* <?=system_showText(LANG_LABEL_ENDDATE)?>:</th>
							<td>
								<table class="tableOrderEvent_date" border="0" cellpadding="0" cellspacing="0">
									<tr>
										<td class="eventDate"><input type="text" name="end_date" id="end_date" value="<?=$end_date?>" /> <span>(<?=format_printDateStandard()?>)</span></td>
									</tr>
								</table>
							</td>
						</tr>
					...
					<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardSIGNUPTable">
						<tr>
							<th colspan="2" class="SIGNUPTable-title"><h2 class="standardSubTitle"><?=system_showText(LANG_BILLINGINFO)?><span>(<?=system_showText(LANG_BILLINGINFO_MSG1)." ".system_showText(LANG_BILLINGINFO_MSG2_EVENT)?>)</span></h2></th>
						</tr>
					</table>
