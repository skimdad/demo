<?
	...
	function search_frontListingSearch($search_for, $section, $mobile = false, $typebronz = false) {
		...
		if (($section == "listing") || ($section == "mobile")) {
			...
			$listing_fields .= ", (SELECT COUNT(id) FROM Review WHERE Review.item_id=Listing_Summary.id) reviews_count";
 ...
 */
		...
		if($section == "listing_results"){
			if ($typebronz) {
				$where_clause[] = (FORCE_SECOND ? "Listing_Summary" : "Listing").".level IN (70) ";
			} else {
				$where_clause[] = (FORCE_SECOND ? "Listing_Summary" : "Listing").".level NOT IN (70) ";
			}
		}
...
/* 			if (isset($search_for["except_level"])) {
		...
		//if ($ltype=="bronz") $where_clause[] = ($section == "listing_results"  || FORCE_SECOND ? "Listing_Summary" : "Listing")."level = 70";
		...
		} elseif ($user_order_by == "az") {
			$searchReturn["order_by"] = ($section == "listing_results"  || FORCE_SECOND ? "Listing_Summary" : "Listing").".title";
		} elseif ($user_order_by == "reviews") {
			$searchReturn["order_by"] = ($section == "listing_results" || FORCE_SECOND ? "Listing_Summary" : "Listing").".avg_review DESC, reviews_count DESC, ".($section == "listing_results" || FORCE_SECOND ? "Listing_Summary" : "Listing").".title, ".($section == "listing_results" || FORCE_SECOND ? "Listing_Summary" : "Listing").".id";
		} elseif ($user_order_by == "distance") {
	...
	function search_frontPromotionSearch($search_for, $section, $soldout = false) {
            ...
            if (($section == "promotion") || ($section == "random") || $section == "promotion_results") {
                $searchReturn["select_columns"] = "DISTINCT Promotion.*,(SELECT COUNT(id) FROM Review WHERE Review.item_id=Promotion.id) reviews_count";
            } elseif ($section == "count") {
                $searchReturn["select_columns"] = "COUNT(Promotion.id),(SELECT COUNT(id) FROM Review WHERE Review.item_id=Promotion.id) reviews_count";
            }
          ...
          /*   if(!$search_for["keyword"] || $section == "count"){
            ...
            if ($user_order_by == "characters") {
                ...
                $searchReturn["order_by"] = "Promotion.name, Promotion.id";
            } elseif ($user_order_by == "reviews") {
                $searchReturn["order_by"] = "Promotion.avg_review DESC,reviews_count DESC, Promotion.name, Promotion.id";
            } elseif ($user_order_by == "distance") {
                $searchReturn["order_by"] = "zipcode_score ASC, Promotion.name, Promotion.id";
			} elseif ($user_order_by == "rating") {
                $searchReturn["order_by"] = "Promotion.avg_review DESC, reviews_count DESC, listing_level ASC, Promotion.name, Promotion.id";
	...
	function search_frontEventSearch($search_for, $section) {
		...
		if ($user_order_by == "characters") {
			...
			$searchReturn["order_by"] = "Event.title, Event.id";
		} elseif ($user_order_by == "distance") {
			$searchReturn["order_by"] = "zipcode_score ASC, Event.title, Event.id";			
			
		} elseif ($user_order_by == "eventdate") {
			$searchReturn["order_by"] = "Event.start_date , Event.start_time , Event.title";
		} elseif ($user_order_by == "popular") {
		...
		if (!$user_order_by) {
			if ((($search_for["keyword"] && $order_by_keyword_score) || ($search_for["where"] && $order_by_where_score) || ($search_for["zip"] && $order_by_zipcode_score)) && ($section != "count") && ($section != "random") && ($user_order_by != LANG_PAGING_ORDERBYPAGE_CHARACTERS) && ($user_order_by != LANG_PAGING_ORDERBYPAGE_LASTUPDATE) && ($user_order_by != LANG_PAGING_ORDERBYPAGE_DATECREATED) && ($user_order_by != LANG_PAGING_ORDERBYPAGE_POPULAR)) {
				$searchReturn["order_by"] = ($order_by_keyword_score2 ? "keyword_score DESC, " : "")." Event.start_date";
				if ($order_by_zipcode_score) {
					$searchReturn["order_by"] .= ", zipcode_score";
				}
				if ($order_by_keyword_score) {
	...
	function search_frontClassifiedSearch($search_for, $section) {
		...
		if (($section == "classified") || ($section == "random") || ($section == "mobile")) {
			$searchReturn["select_columns"] = "Classified.* , (SELECT COUNT(id) FROM Review WHERE Review.item_id=Classified.id) reviews_count";
		} elseif ($section == "count") {
		...
		if ($user_order_by == "characters") {
			...
			$searchReturn["order_by"] = "Classified.title, Classified.id";
		} elseif ($user_order_by == "distance" && $_GET["zip"] != "") {
			$searchReturn["order_by"] = "zipcode_score ASC, Classified.title, Classified.id";			
		...
		if (!$user_order_by) {
			if ((($search_for["keyword"] && $order_by_keyword_score) || ($search_for["where"] && $order_by_where_score) || ($search_for["zip"] && $order_by_zipcode_score)) && ($section != "count") && ($section != "random") && ($user_order_by != LANG_PAGING_ORDERBYPAGE_CHARACTERS) && ($user_order_by != LANG_PAGING_ORDERBYPAGE_LASTUPDATE) && ($user_order_by != LANG_PAGING_ORDERBYPAGE_DATECREATED) && ($user_order_by != LANG_PAGING_ORDERBYPAGE_POPULAR)) {
				$searchReturn["order_by"] = ($order_by_keyword_score2 ? "keyword_score DESC, " : "")."Classified.zipcode_score";
				if ($_GET["zip"] != "") $searchReturn["order_by"] = "zipcode_score ASC"; else $searchReturn["order_by"] = "Classified.title, Classified.id";
				if ($order_by_zipcode_score) {
					$searchReturn["order_by"] .= ", zipcode_score";
				}
				if ($order_by_keyword_score) {
	...
	function search_frontArticleSearch($search_for, $section) {
		...
		$searchReturn["select_columns"] .= ", (SELECT COUNT(id) FROM Review WHERE Review.item_id=Article.id) reviews_count";
		...
		if ($user_order_by == "characters") {
			...
			$searchReturn["order_by"] = "Article.title, Article.id";
		} elseif ($user_order_by == "reviews") {
			$searchReturn["order_by"] = "reviews_count DESC, Article.title, Article.id";
	...
	function search_getOrderbyDropDown ($getData, $pagingUrl, $orderBy, $defaultText = "Sort by: ", $defaultOnChange = "this.form.submit();", $array_params = false, $deal = false, $blog = false, $classified = false, $event = false) {
...
/* 		if (!$blog && !$classified && !$event){
			$orderbyDropDown .= "<option value=\"\">".LANG_PAGING_ORDERBYPAGE_REVIEWS."</option>\n";
		}  */
		//if ($classified) { $orderbyDropDown .= "<option value=\"\">".LANG_PAGING_ORDERBYPAGE_DISTANCE."</option>\n";}
		if ($event && false) { $orderbyDropDown .= "<option value=\"\">".LANG_PAGING_ORDERBYPAGE_EVENTDATE."</option>\n";}		
		...
		for ($i = 0; $i < count($orderBy); $i++) {
			...
			elseif ($orderBy[$i] == LANG_PAGING_ORDERBYPAGE_EVENTDATE) { $option_value = "eventdate"; }
...
//wtc
			elseif ($orderBy[$i] == LANG_PAGING_ORDERBYPAGE_REVIEWS) { $option_value = "reviews"; }			
			elseif ($orderBy[$i] == LANG_PAGING_ORDERBYPAGE_DISTANCE) { $option_value = "distance"; }
			elseif ($orderBy[$i] == LANG_PAGING_ORDERBYPAGE_AZ) { $option_value = "az"; }
