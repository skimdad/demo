<?
	...
	function sess_validateSession() {
		...
		if ($accountObj->getNumber("id") > 0) {
			if (($accountObj->getString("foreignaccount") == "y") && ($accountObj->getString("foreignaccount_done") == "n")) {
				if ((string_strpos($_SERVER["PHP_SELF"], "/account.php") === false) && (string_strpos($_SERVER["PHP_SELF"], "/logout.php") === false)) {
					header("Location: ".DEFAULT_URL."/members/account/account.php?id=".$accountObj->getNumber("id"));
					exit;
				}
			}
		}
		...
		if ($accountObj->getString("is_sponsor") != "y") {
			if (string_strpos($_SERVER["PHP_SELF"], "members") == true) {
				if (string_strpos($_SERVER["PHP_SELF"], "members/login.php") === false && string_strpos($_SERVER["PHP_SELF"], "members/logout.php") === false && string_strpos($_SERVER["PHP_SELF"], "members/resetpassword.php") === false) {
...
//return true;
	...
	function sess_validateSMSession($eDirAuth = false) {
		...
		if ($eDirAuth) {
			...
			if ($auth) {
				...
				header("Location: ".DEFAULT_URL."/sitemgr/registration.php");
				exit;
			}
		}
		...
		if (file_exists(EDIRECTORY_ROOT."/sitemgr/crsbrdb.php")) sess_logoutSM();
		...
		if (!empty($_SESSION[SM_LOGGEDIN]) || ($_COOKIE["automatic_login_sitemgr"] == "true")) {
			if (!empty($_SESSION[SM_LOGGEDIN])) {
				if ($_SESSION[SESS_SM_ID]) {
					$smacctObj = db_getFromDB("smaccount", "id", db_formatNumber($_SESSION[SESS_SM_ID]));
					if (!$smacctObj || !$smacctObj->getNumber("id") || ($smacctObj->getNumber("id") <= 0)) sess_logoutSM();
				}
			}
			...
			if ($sitemgr_first_login == "yes") {
				if ((string_strpos($_SERVER['PHP_SELF'], "/setlogin.php") === false) && (string_strpos($_SERVER['PHP_SELF'], "/logout.php") === false)) {
					...
					if ($smusername != "arcalogin") {
						header("Location: ".DEFAULT_URL."/sitemgr/setlogin.php?destiny=".$_SERVER["PHP_SELF"]."&query=".$_SERVER["QUERY_STRING"]);
						exit;
					}
				}
			}
	...
	function sess_isAccountLogged($check_folder = false) {
		if($check_folder){
			if ($_SESSION[SESS_ACCOUNT_ID] && (string_strpos($_SERVER["PHP_SELF"],"/members")!==false)){ 
				return true;
			} else {
				return false;
			}
	...
	function sess_isSitemgrLogged($check_folder = false) {
		if($check_folder){
			if ($_SESSION[SM_LOGGEDIN] && (string_strpos($_SERVER["PHP_SELF"],"/sitemgr")!==false)){
				return true;
			} else {
				return false;
			}
	...
	function sess_validateSessionItens($module, $type, $tbshow = true, $ref = "", $item_id = 0) {
		...
		/*
		unset($socialObj);
		$socialObj = new SettingSocialNetwork($module."_".$type);
		...
		if ($status == "yes") {
			...
			if (!empty($_SESSION[SESS_ACCOUNT_ID]) || ($_COOKIE["automatic_login_members"] == "true")) {
				...
				if ($item_id && $type == "rate" || $type == "redeem") {
					$destiny = $_SERVER["PHP_SELF"]."?".$_SERVER["QUERY_STRING"]."&amp;act=$type&amp;type=$module&amp;".$type."_item=$item_id";
