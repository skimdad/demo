<?
	...
	function validate_date($date, $default_lang = false) {
		
		if ($default_lang) $default_date_format = system_findTranslationFor("DEFAULT_DATE_FORMAT", EDIR_DEFAULT_LANGUAGE);
		else $default_date_format = DEFAULT_DATE_FORMAT;
	...
	function validate_pastDate($date) {
		...
		if (count($aux) == 3 ) {
			...
			if (is_numeric($aux[0]) && is_numeric($aux[1]) && is_numeric($aux[2])) {
				...
				if (checkdate((int)$month, (int)$day, (int)$year)) {
					string_strlen($month)==1?$month="0".$month:$month=$month;
					string_strlen($day)==1?$day="0".$day:$day=$day;
	...
	*/
	...
	function validate_form($form, $array, &$error, $newcategories='', $bannerErrorSize = 0) {
		...
		elseif ($form == 'navbarItem'){
			if (!trim(${"name_".$lang_id}))
                $errors[] = "&#149;&nbsp; ".system_showText(LANG_SITEMGR_MENUCONFIG_ENTERLABEL);
			if (!$link) $errors[] = "&#149;&nbsp; ".system_showText(LANG_SITEMGR_MENUCONFIG_ENTERURL);
			if ($link && !domain_validateNavbarUrl($link)) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_ENTER_VALID_URL);
		}
		...
		elseif ($form == "discountcode") {
			...
			if (!preg_match("/^([0-9a-zA-Z_]{1,10})/", $id))	$errors[] = "&#149;&nbsp;".system_showText(LANG_SITEMGR_MSGERROR_CODENEEDSTOBEANALPHANUMERIC)."";
		...
		elseif ($form == "contact") {
			if (!$lang)       $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_LANGUAGE_IS_REQUIRED);
		...
		elseif ($form == "listing") {
			...
			if ($newcategories || $newcategories=='signup')
				$elems[] = array('name'=>'return_categories', 'label'=>system_showText(LANG_LABEL_CATEGORY_PLURAL),     'type'=>'text',   'required'=>false,                                        );
			else
			...
			if ($newcategories && $newcategories!='signup'){
				if ($return_categories_array[0])
					$cont=count($return_categories_array)+count($newcategories);
				else
					$cont=count($return_categories_array)+count($newcategories)-1;
				if($cont > LISTING_MAX_CATEGORY_ALLOWED) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_MAX_OF_CATEGORIES_1)." ".LISTING_MAX_CATEGORY_ALLOWED." ".system_showText(LANG_MSG_MAX_OF_CATEGORIES_2);
			}else{
		...
		elseif ($form == "classified") {
			...
			if ($newcategories){
				if ($return_categories_array[0])
					...
					$cont=count($return_categories_array)+count($newcategories)-1;
				if($cont > MAX_CATEGORY_ALLOWED) $errors[] = system_showText(LANG_MSG_MAX_OF_CATEGORIES_1)." ".MAX_CATEGORY_ALLOWED." ".system_showText(LANG_MSG_MAX_OF_CATEGORIES_2);
			}else{
		...
		elseif ($form == "article") {
			...
			if (!$signup) if (!${'abstract'.EDIR_DEFAULT_LANGUAGENUMBER}) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_ABSTRACT_IS_REQUIRED);
		...
		elseif ($form == "articleseocenter") {
			...
			if ($expiration_setting == BANNER_EXPIRATION_IMPRESSION && (!$unpaid_impressions || $unpaid_impressions == 0) && !$id && !string_strpos($_SERVER["PHP_SELF"], "sitemgr")) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_IMPRESSIONS_ARE_REQUIRED);
			if ($expiration_setting == BANNER_EXPIRATION_IMPRESSION && (!$impressions || $impressions == 0) && !$id && string_strpos($_SERVER["PHP_SELF"], "sitemgr")) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_IMPRESSIONS_ARE_REQUIRED);

			if (!$type && !$id) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_TYPE_IS_REQUIRED);

			if (!${"caption".EDIR_DEFAULT_LANGUAGENUMBER}) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_CAPTION_IS_REQUIRED);

			if ($bannerErrorSize) {
				$errors[] = "&#149;&nbsp;".system_showText(LANG_UPLOAD_MSG_EXCEEDSLIMIT);
			} else {
				if (!$signup && $show_type!=1) {
					if (!${"file".EDIR_DEFAULT_LANGUAGENUMBER} && $type < 50 && !$id) {
						$errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_FILE_IS_REQUIRED);
					} elseif (!${"file".EDIR_DEFAULT_LANGUAGENUMBER} && $type < 50 && $id) { // required because of the different type approachs (image banner and text banner)
						$bannerObj = new Banner($id);
						if(!$bannerObj->getNumber("image_id".EDIR_DEFAULT_LANGUAGENUMBER)) { $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_FILE_IS_REQUIRED); }
						unset($bannerObj);
					} else if (!${"image_id".EDIR_DEFAULT_LANGUAGENUMBER} && $type < 50) {
						$errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_FILE_IS_REQUIRED);
					}
				}
			}
			...
			if($show_type==1 && !$signup) {
				...
				if (!${"content_line1".EDIR_DEFAULT_LANGUAGENUMBER} && $type >= 50)	$errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_DESCRIPTION1_IS_REQUIRED);
				if (!${"content_line2".EDIR_DEFAULT_LANGUAGENUMBER} && $type >= 50)	$errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_DESCRIPTION2_IS_REQUIRED);
			}
			...
			if (${"title".EDIR_DEFAULT_LANGUAGENUMBER}) {
				$sql = "SELECT title".EDIR_DEFAULT_LANGUAGENUMBER." FROM $table_category WHERE title".EDIR_DEFAULT_LANGUAGENUMBER." = ".db_formatString(trim(${"title".EDIR_DEFAULT_LANGUAGENUMBER}))."";
			...
			if (${"page_title".EDIR_DEFAULT_LANGUAGENUMBER}) {
				$sql = "SELECT page_title".EDIR_DEFAULT_LANGUAGENUMBER." FROM $table_category WHERE page_title".EDIR_DEFAULT_LANGUAGENUMBER." = ".db_formatString(trim(${"page_title".EDIR_DEFAULT_LANGUAGENUMBER}))."";
				$sql .= ($category_id) ? " AND category_id = $category_id " : " AND category_id = 0 ";
				if ($id) $sql .= " AND id != $id ";
				$sql .= " LIMIT 1";
				$rs = $dbObj->query($sql);
			...
			if (${"friendly_url".EDIR_DEFAULT_LANGUAGENUMBER}) {
				$sql = "SELECT friendly_url".EDIR_DEFAULT_LANGUAGENUMBER." FROM $table_category WHERE friendly_url".EDIR_DEFAULT_LANGUAGENUMBER." = ".db_formatString(trim(${"friendly_url".EDIR_DEFAULT_LANGUAGENUMBER}))."";
				$sql .= ($category_id) ? " AND category_id = $category_id " : " AND category_id = 0 ";
				if ($id) $sql .= " AND id != $id ";
				$sql .= " LIMIT 1";
				$rs = $dbObj->query($sql);
			...
			foreach ($language_numbers as $i) {
				if ($array_lang) {
					if (in_array($array_edir_languages[$k], $array_lang)) {
						...
						if (${"title".$i}) {
							$sql = "SELECT title".$i." FROM $table_category WHERE title".$i." = ".db_formatString(trim(${"title".$i}))."";
							$sql .= ($category_id) ? " AND category_id = $category_id " : " AND category_id = 0 ";
							if ($id) $sql .= " AND id != $id ";
							$sql .= " LIMIT 1";
							$rs = $dbObj->query($sql);
						...
						if (${"page_title".$i}) {
							$sql = "SELECT page_title".$i." FROM $table_category WHERE page_title".$i." = ".db_formatString(trim(${"page_title".$i}))."";
							$sql .= ($category_id) ? " AND category_id = $category_id " : " AND category_id = 0 ";
							if ($id) $sql .= " AND id != $id ";
							$sql .= " LIMIT 1";
							$rs = $dbObj->query($sql);
						...
						if (${"friendly_url".$i}) {
							$sql = "SELECT friendly_url".$i." FROM $table_category WHERE friendly_url".$i." = ".db_formatString(trim(${"friendly_url".$i}))."";
							$sql .= ($category_id) ? " AND category_id = $category_id " : " AND category_id = 0 ";
							if ($id) $sql .= " AND id != $id ";
							$sql .= " LIMIT 1";
							$rs = $dbObj->query($sql);
			...
			for ($i=0; $i<count(explode(",", EDIR_LANGUAGES)); $i++) {
				...
				if (${"array_keywords".$labelsuffix}) {
					if (count(${"array_keywords".$labelsuffix}) > MAX_KEYWORDS) {
						$err[] = "&#149;&nbsp;".system_showText(LANG_MSG_MAX_OF_KEYWORDS_ALLOWED_1)." ".MAX_KEYWORDS." ".system_showText(LANG_MSG_MAX_OF_KEYWORDS_ALLOWED_2)." (".$array_edir_languagenames[$i].")";
					}
					$kwlarge = false;
					foreach (${"array_keywords".$labelsuffix} as $kw) {
						if (string_strlen($kw) > 50) {
					...
					if ($kwlarge) {
						$err[] = "&#149;&nbsp;".system_showText(LANG_MSG_PLEASE_INCLUDE_KEYWORDS)." (".$array_edir_languagenames[$i].")";
					}
				}
			}
...
//								if (!validate_NumberWeek($start_date,$until_date,$weekarray[$i])){
//									$dateTimeError = true;
//									$err[] = system_showText(LANG_EVENT_CHECK_WEEK);
//									break;
//								}
//							}
//						}
...
//						if (!$dateTimeError){
//							if (!validate_NumberWeek($start_date,$end_date,$week)){
//								$dateTimeError = true;
//								$err[] = system_showText(LANG_EVENT_CHECK_WEEK);
//							}
//						}
			...
			if ($newcategories){
				if ($return_categories_array[0])
					...
					$cont=count($return_categories_array)+count($newcategories)-1;
				if($cont > MAX_CATEGORY_ALLOWED) $err[] = system_showText(LANG_MSG_MAX_OF_CATEGORIES_1)." ".MAX_CATEGORY_ALLOWED." ".system_showText(LANG_MSG_MAX_OF_CATEGORIES_2);
			}else{
				if(count($return_categories_array) > MAX_CATEGORY_ALLOWED) $err[] = system_showText(LANG_MSG_MAX_OF_CATEGORIES_1)." ".MAX_CATEGORY_ALLOWED." ".system_showText(LANG_MSG_MAX_OF_CATEGORIES_2);
			}
		...
		elseif ($form == "domain") {
			...
			if ($offer_item == "custom_package"){
				...
				if (!${"content".EDIR_DEFAULT_LANGUAGENUMBER}) $errors[] = "&#149;&nbsp;".system_showText(LANG_SITEMGR_MSGERROR_CONTENTISREQUIRED)."";
			} else {
	...
	function validate_listingtemplate($array, &$error, $validateAll = true) {
		...
		if ($friendly_url) {
			$dbMain = db_getDBObject(DEFAULT_DB,true);
			$dbObj = db_getDBObjectByDomainID(SELECTED_DOMAIN_ID,$dbMain);
			$sql = "SELECT friendly_url FROM ListingTemplate WHERE friendly_url = ".db_formatString($friendly_url)." AND editable = 'y'";
			if ($id) $sql .= " AND id != $id ";
			$sql .= " LIMIT 1";
			$rs = $dbObj->query($sql);
			if (mysql_num_rows($rs) > 0) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_FRIENDLY_URL_IN_USE);
	...
	function validate_MEMBERS_account($array, &$error) {
		extract($array);
		$error = "";
		if ((string_strlen($password))||(string_strlen($retype_password))) $error = validate_password($password, $retype_password, true);
		if ($error) {
			return false;
		}
		return true;
