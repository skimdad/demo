...
?>
    ...
    <? if (FEATURED_LOCATION == "on") {
		...
		// CREATING CACHE
		$objCache = new cache("sidebar_location_event");
			...
			if ($locationsToShow) {
				...
				if ($found_featLocations) { ?>
					...
					<ul id="accordion">
						...
						foreach($locationsToShow as $_location_level) {
							...
							if (count(${"locations".$_location_level}) && $count_locationFeatLevels<=FEATUREDLOCATION_LEVEL_AMOUNT) { ?>
								<li class="accordion-item">
									<h3><a href="#events"><?=system_showText((constant("LANG_LABEL_".constant("LOCATION".$_location_level."_SYSTEM")."_PL")))?></a></h3>
									<ul class="list <?=$count_locationFeatLevels == 1 ? "current" : ""?>">
										...
										foreach (${"locations".$_location_level} as $each_location) {
											...
											if (MODREWRITE_FEATURE != "on") { ?>
												<li><a href="<?=EVENT_DEFAULT_URL?>/results.php?<?=($location_path["nonfriendly"] === false ? '' : implode("&",$location_path["nonfriendly"])."&" )?>location_<?=$_location_level?>=<?=$each_location["id"];?>"><?=$each_location["name"];?></a></li>
											<? } else { ?>
												<li><a href="<?=EVENT_DEFAULT_URL?>/location/<?=($location_path["friendly"] === false ? '' : implode("/",$location_path["friendly"])."/" )?><?=$each_location["friendly_url"];?>"><?=$each_location["name"];?></a></li>
											<? }
											unset ($location_path);
										}
										?>
						...
						?>
							<li class="view-all"><a href="<?=EVENT_DEFAULT_URL?>/alllocations.php"><?=system_showText(LANG_VIEWALLLOCATIONSCATEGORIES)?></a></li>
					</ul>		
					<?
				}
