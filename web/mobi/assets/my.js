...
function Directory_account(){
    $(".sign_in").hide();
    $(".Directory_account").show();
}
...
function selectmenu1_change(this_elem){
    var selected_item = this_elem.val();
        
    if(selected_item == 'acc') {
        // $(".sign_in").hide();
        // $(".Directory_account").show();
        Directory_account();        
    }
...
$(document).bind( "pageinit", function( event, data ){ 
    ...
    $('.openpoup').click(function(){
        ...
        if(this_element.hasClass('fb_login')){
             //alert('fb_login');
                 window.location.href = this_element.attr('href');
             return false;
        }
        $.ajax({
            url: this_element.attr('href'),
            type: 'GET',
            success: function(response){
              // alert(response);
               respons_obj = $(response);
               respons_obj.trigger('create');
               this_element.simpledialog2({
                    'mode' : 'blank',
                    'prompt': false,
                    'forceInput': false,
                    'useModal':true,
                    'blankContent' : respons_obj.html()
                });  
            }
...
$(document).bind ('pageshow', function (e, data) {
    //$('.ui-simpledialog-screen').click(function(){  $('.redeem-detail').simpledialog('close');});

    if($('.ui-page-active .splash_image_container').length > 0)
    {
        $('.ui-page-active .splash_image').load(function(){
            setTimeout(function(){
                $.mobile.changePage(home_url);
            }, 2000)
        });
        if($('.ui-page-active .splash_image').length == 0)
        {
            setTimeout(function(){
                $.mobile.changePage(home_url);
            }, 200)
    ...
    if($('.ui-page-active .redeem-detail').length > 0)
    {
        redeem_html = $('.redeem-detail').html();
        
        redeem_obj = $(redeem_html) .trigger('create');

        $('.redeem-detail').simpledialog2(/*{
                'mode' : 'blank',
                'prompt': false,
                'forceInput': false,
                'useModal':true,
                'fullHTML' : redeem_obj.html()
            }*/);  
    }
