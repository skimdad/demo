				...
				<th>
					<fieldset title="<?=$each_listing["listing_title"]?>">
						...
						if ($transactionListingObj->getNumber("id") > 0) {
							?><a href="<?=$url_base?>/<?=LISTING_FEATURE_FOLDER;?>/view.php?id=<?=$each_listing["listing_id"]?>" class="link-table">
				...
				<th>
					<fieldset title="<?=$each_event["event_title"]?>">
						...
						if ($transactionEventObj->getNumber("id") > 0) {
							?><a href="<?=$url_base?>/<?=EVENT_FEATURE_FOLDER;?>/view.php?id=<?=$each_event["event_id"]?>" class="link-table">
				...
				<th>
					<fieldset title="<?=$each_banner["banner_caption"]?>">
						...
						if ($transactionBannerObj->getNumber("id") > 0) {
							?><a href="<?=$url_base?>/<?=BANNER_FEATURE_FOLDER;?>/view.php?id=<?=$each_banner["banner_id"]?>" class="link-table">
				...
				<th>
					<fieldset title="<?=$each_classified["classified_title"]?>">
						...
						if ($transactionClassifiedObj->getNumber("id") > 0) {
							?><a href="<?=$url_base?>/<?=CLASSIFIED_FEATURE_FOLDER;?>/view.php?id=<?=$each_classified["classified_id"]?>" class="link-table">
				...
				<th>
					<fieldset title="<?=$each_article["article_title"]?>">
						...
						if ($transactionArticleObj->getNumber("id") > 0) {
							?><a href="<?=$url_base?>/<?=ARTICLE_FEATURE_FOLDER;?>/view.php?id=<?=$each_article["article_id"]?>" class="link-table">
					...
					<fieldset title="<?=$each_custominvoice["title"]?>">
						...
						if ($transactionCustomInvoiceObj->getNumber("id") > 0) {
							if (string_strpos($url_base, "/sitemgr") !== false) {
								?><a href="<?=$url_base?>/custominvoices/view.php?id=<?=$each_custominvoice["custom_invoice_id"]?>" class="link-table">
...
<? if ($transaction_package_log) { ?>
	...
	<table align="center" border="0" cellpadding="2" cellspacing="2" class="standard-innerTable">
		<tr>
			<th class="tableTitle" colspan="2"><p><?=system_showText(LANG_PACKAGE_SING);?></p></th>
		</tr>
				...
				<th>
					<fieldset title="<?=$each_package["title"]?>">
						...
						if ($transactionPackageObj->getNumber("id") > 0) {
							if (string_strpos($url_base, "/sitemgr") !== false) {
								?><a href="<?=$url_base?>/package/view.php?id=<?=$each_package["package_id"]?>" class="link-table">
