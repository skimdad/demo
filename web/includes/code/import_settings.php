<?
	...
	if (!isset($import_from_export)) system_setFreqActions('prefs_import','import');
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if(!setting_set("import_automatic_start", $import_automatic_start))
			if(!setting_new("import_automatic_start", $import_automatic_start))
				$error = true;
		...
		if (EVENT_FEATURE == "on" && CUSTOM_EVENT_FEATURE == "on") {
			...
			if(!setting_set("import_automatic_start_event", $import_automatic_start_event))
				if(!setting_new("import_automatic_start_event", $import_automatic_start_event))
					$error = true;
	...
	if ($import_automatic_start) $import_automatic_start = "checked";
	...
	if (EVENT_FEATURE == "on" && CUSTOM_EVENT_FEATURE == "on"){
		...
		if ($import_automatic_start_event) $import_automatic_start_event = "checked";
