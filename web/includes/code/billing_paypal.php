<?
	...
	if (string_strpos($_SERVER["PHP_SELF"], "receipt.php") === false) {
		...
		if ($txn_id || $subscr_id || $receipt_id || $tx){
			...
			if ($process == "signup") $try_again_message = "<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=paypal\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
			elseif ($process == "claim") $try_again_message = "<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=paypal&claimlistingid=".$claimlistingid."\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
