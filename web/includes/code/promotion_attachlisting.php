	...
	/*	$act_id = ($_GET["account_id"]==0) ? "" : " AND `account_id` = ".$_GET["account_id"];
         $sqlListings = "SELECT `id`, 
			...
			*/
				...
				$act_id = ($_GET["account_id"]==0) ? "" : "  `account_id` = ".$_GET["account_id"];
          $sqlListings = "SELECT `id`, 
		...
		$listid= $rowListings["id"];
		$level = $rowListings["level"];
		//echo "select free_deal from ListingLevel where value=".$level." limit 1";
		$query1=mysql_query("select free_deal from ListingLevel where value=".$level." limit 1");
		
		  while ($levellist = mysql_fetch_assoc($query1)) {
			$free_deal = $levellist["free_deal"];
		  }
		
		//echo "select listing_id from Promotion where listing_id=".$listid;
		
		$query2=mysql_query("select listing_id  from Promotion where listing_id=".$listid);
		$num_rows2 = mysql_num_rows($query2);
		if($num_rows2<=$free_deal)
