<?
	...
	if ($pay) {
		...
		if (!is_array($listing_id) && !is_array($event_id) && !is_array($banner_id) && !is_array($classified_id) && !is_array($article_id) && !is_array($custominvoice_id)) {
			...
			if ($process == "signup") $payment_message .= system_showText(LANG_MSG_SYSTEM_FAILURE_TRY_AGAIN)."<br />\n<br />\n<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=paypalapi\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
			elseif ($process == "claim") $payment_message .= system_showText(LANG_MSG_SYSTEM_FAILURE_TRY_AGAIN)."<br />\n<br />\n<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=paypalapi&claimlistingid=".$claimlistingid."\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
		...
		} elseif ($zip && (!is_numeric($zip) || ((string_strlen($zip) != 5) && (string_strlen($zip) != 9)))) {
	...
	if ($pay && $verification == "y") {
		...
		include(EDIRECTORY_ROOT."/members/billing/CallerService.php");
		...
		if (!$resArray["CURL_ERROR"]) {
			...
			if ($ack != "SUCCESS") {
				...
				if ($process == "signup") $try_again_message = "<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=paypalapi\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
				elseif ($process == "claim") $try_again_message = "<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=paypalapi&claimlistingid=".$claimlistingid."\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
				...
				else $payment_message .= "<br />\n".system_showText(LANG_MSG_INFO_ABOUT_TRANSACTION_MAY_BE_FOUND)."<br />\n".system_showText(LANG_MSG_IN_YOUR)." <a href=\"".DEFAULT_URL."/members/transactions/index.php\">".system_showText(LANG_LABEL_TRANSACTION_HISTORY)."</a><br />\n";
