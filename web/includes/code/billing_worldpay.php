<?
	...
	if (string_strpos($_SERVER["PHP_SELF"], "callback.php") === false) {
		...
		else $payment_message .= system_showText(LANG_MSG_IF_TRANSACTION_WAS_CONFIRMED)."<br />\n".system_showText(LANG_LABEL_YOUR)." <a href=\"".DEFAULT_URL."/members/transactions/index.php\">".system_showText(LANG_LABEL_TRANSACTION_HISTORY)."</a> ".system_showText(LANG_MSG_AFTER_PAYMENT_IS_PROCESSED)."<br />\n(".system_showText(LANG_MSG_TRANSACTIONS_MAY_NOT_OCCUR_IMMEDIATELY).")\n";
		...
		if ($transaction_worldpay["transStatus"]) {
			...
			if ($transaction_worldpay["transStatus"] != "Y") {
				...
				if ($process == "signup") $try_again_message = "<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=worldpay\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
				elseif ($process == "claim") $try_again_message = "<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=worldpay&claimlistingid=".$claimlistingid."\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
                ...
                else $payment_message .= "<br />\n".system_showText(LANG_MSG_TRANSACTIONS_MAY_NOT_OCCUR_IMMEDIATELY)."<br />\n".system_showText(LANG_MSG_AFTER_PAYMENT_IS_PROCESSED_INFO_ABOUT)."<br />\n".system_showText(LANG_MSG_MAY_BE_FOUND_IN_YOUR)." <a href=\"".DEFAULT_URL."/members/transactions/index.php\">".system_showText(LANG_LABEL_TRANSACTION_HISTORY)."</a>\n";
				...
				if (!empty($listing_ids[0])) {
					...
					$amountAux = 0;
					$levelObj = new ListingLevel(EDIR_DEFAULT_LANGUAGE, false, $domain_id);
				...
				if (!empty($event_ids[0])) {
					...
					foreach($event_ids as $each_event_id){
                        ...
                        $levelObj = new EventLevel(EDIR_DEFAULT_LANGUAGE, false, $domain_id);
				...
				if (!empty($banner_ids[0])) {
					...
					foreach($banner_ids as $each_banner_id){
                        ...
                        $levelObj = new BannerLevel(EDIR_DEFAULT_LANGUAGE, false, $domain_id);
				...
				if (!empty($classified_ids[0])) {
					...
					foreach($classified_ids as $each_classified_id){
                        ...
                        $levelObj = new ClassifiedLevel(EDIR_DEFAULT_LANGUAGE, false, $domain_id);
				...
				if (!empty($article_ids[0])) {
					...
					foreach($article_ids as $each_article_id){
                        ...
                        $levelObj = new ArticleLevel(EDIR_DEFAULT_LANGUAGE, false, $domain_id);
