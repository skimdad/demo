...
//			$discountCodeObj->expire_date = format_date($discountCodeObj->expire_date);
		...
		if ((validate_form("discountcode", $_POST, $message_discountcode)) && (!is_repeated_code($message_discountcode))) {
			...
			if($_POST["x_id"]){ // update
				...
				//$message = string_ucwords(LANG_LABEL_DISCOUNTCODE)." &quot;".$_POST["x_id"]."&quot; ".system_showText(LANG_SITEMGR_SUCCESSUPDATED);
                $message = 4;
			} else { // insert
				$discountCodeObj = new DiscountCode($_POST);
				//$message = string_ucwords(LANG_LABEL_DISCOUNTCODE)." &quot;".$_POST["id"]."&quot; ".system_showText(LANG_SITEMGR_SUCCESSADDED);
                $message = 5;
			}
