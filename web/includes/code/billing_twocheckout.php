	...
	if($x_response_code){
		...
		if ($transaction_twocheckout["response_code"]) {
			...
			if (($transaction_twocheckout["response_code"] && $transaction_twocheckout["response_code"] == "2") || ($x_2checked != "Y")) { // transaction failed.
				...
				if ($process == "signup") $try_again_message = "<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=twocheckout\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
				elseif ($process == "claim") $try_again_message = "<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=twocheckout&claimlistingid=".$claimlistingid."\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
