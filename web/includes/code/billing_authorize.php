<?
	...
	if ($pay) {
		...
		if (AUTHORIZERECURRING_FEATURE == "on") {
			if ($x_cc_year_exp_date == date("y") && AUTHORIZE_RECURRINGLENGTH == "1") {
				$installments = $x_cc_month_exp_date - date("m");
			} else {
				if (AUTHORIZE_RECURRINGLENGTH == "1") $installments = (12 - date("m")) + (($x_cc_year_exp_date - date("y") - 1) * 12) + $x_cc_month_exp_date;
				elseif (AUTHORIZE_RECURRINGLENGTH == "12") {
				    if ( ($x_cc_month_exp_date - date("m")) >= 0 ) $sub = 0;
				    else $sub = -1;
					$installments = ($x_cc_year_exp_date - date("y")) + $sub;	
				}  
			}
		}
		...
		if ($installments < 0) {
			$validationok = "no";
			if ($process == "signup") $payment_message = "<p class=\"errorMessage\">".system_showText(LANG_MSG_WRONG_CARD_EXPIRATION_TRY_AGAIN)."<br />\n<br />\n<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=authorize\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a></p>\n";
			elseif ($process == "claim") $payment_message = "<p class=\"errorMessage\">".system_showText(LANG_MSG_WRONG_CARD_EXPIRATION_TRY_AGAIN)."<br />\n<br />\n<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=authorize&claimlistingid=".$claimlistingid."\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a></p>\n";
		...
		if ($pay && $validationok == "yes") {
			...
			if ($curl_response) {
				...
				if ($transaction_status == "ok") {
					...
					if ($process == "signup") $try_again_message = "\n<a href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=authorize\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
					elseif ($process == "claim") $try_again_message = "\n<a style=\"cursor:pointer\" href=\"".DEFAULT_URL."/members/".$process."/payment.php?payment_method=authorize&claimlistingid=".$claimlistingid."\">".system_showText(LANG_MSG_CLICK_HERE_TO_TRY_AGAIN)."</a>\n";
