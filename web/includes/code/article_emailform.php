<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /includes/code/article_emailform.php
	# ----------------------------------------------------------------------------------------------------
    ...
    if ($id) {
        $check_id = db_getFromDB('article', 'id', $id);
        if (!$check_id->id)  {
            $error = system_showText(LANG_MSG_TOFRIEND3)." <a href=".DEFAULT_URL.">" .DEFAULT_URL." </a>".system_showText(LANG_MSG_TOFRIEND4)."<br />";
            $item_error = TRUE;
        }
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (empty($error)) {
			...
			if ($receiver == "friend") {
				...
				if ($obj->getString("abstract".$labelsuffix)) $message .= " ".$obj->getString("abstract".$labelsuffix);
