	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if ($type == "upload" || $type == "select") {
			...
			if (!$messageErrorUpload) {
				...
				$importlogObj->setString("update_itens", ($import_update == "1"? "y" : "n"));
				$importlogObj->Save();
				...
				if ($ftp_type != "schedule_cron") {
					...
					if ($import_stop) {
						...
						import_generateSQLLot($importID, $db, "event");
					}
				}

				if ($upload_name) $aType = 1; //upload
				else if ($file_name) $aType = 2; //from ftp
				header("Location: ".DEFAULT_URL."/sitemgr/import/importlog.php?import_type=event&log_id=".$importID."&type=".$aType);
				exit;
			}
			...
			if ($option == "verify_lines") {
				...
				$sqlL = "SELECT `id`, `date`, `time`, `filename`, `linesadded`, `phisicalname`, `status`, `action`, `progress`, `totallines`, `history`, `update_itens`, `delimiter`, `mysqlerror` FROM `ImportLog` WHERE `id` = ".$log_id." AND type = 'event'";
