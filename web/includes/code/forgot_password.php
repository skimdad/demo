	...
	if (!$_POST["username"]) {
		...
		if (!$user_info) {
			...
			if ($section == "sitemgr") {
				...
				### SITEMGR EMAIL
				##################################################
				$link = DEFAULT_URL."/sitemgr/login.php?key=".$row["unique_key"];
				setting_get("sitemgr_email", $sitemgr_email);
				...
				if (!$emailNotificationObj = system_checkEmail(SYSTEM_FORGOTTEN_PASS, $contactObj->getString("lang"))) {
					...
					if ($accountObj->getString("is_sponsor") == "y") {
						$link = DEFAULT_URL."/members/login.php?key=".$row["unique_key"];
					} else {
						$link = SOCIALNETWORK_URL."/login.php?key=".$row["unique_key"];
					}
					setting_get("sitemgr_email", $sitemgr_email);
