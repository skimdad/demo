...
?>
	    <tr>
		    <th colspan="<?=TWILIO_APP_ENABLED == "on" ? (TWILIO_APP_ENABLED_CALL == "on" && TWILIO_APP_ENABLED_SMS == "on" ? "9" : "8") : "7"?>">
                <?
                    if(string_strpos($_SERVER['REQUEST_URI'], "/sitemgr") !== false) {
	    ...
	    <tr>
			...
			<td width="124">
			    <b style="color: #CC3300;">
					...
					<? if(string_strpos($_SERVER['REQUEST_URI'], "/sitemgr") !== false) { ?>
						<a href="<?=DEFAULT_URL?>/sitemgr/<?=LISTING_FEATURE_FOLDER?>/twilio_report.php?item_type=listing&item_id=<?=$listing->getNumber("id")?>" class="iframe fancy_window">
					<? } else {?>
						<a href="<?=DEFAULT_URL?>/popup/popup.php?pop_type=twilio_report&item_type=listing&item_id=<?=$listing->getNumber("id")?>" class="iframe fancy_window_twilio_reports">
					<? } ?>
...
?>
    <script language="JavaSCRIPT">
        function changeChart(idx, value1, value2, value3, value4, value5, value6, value7, value8) {
            ...
            document.getElementById("reportChart").innerHTML = "<img src='http://chart.apis.google.com/chart?chs=700x200&amp;chf=bg,s,ffffff|c,s,ffffff&amp;chxt=x,y&amp;chxl=1:||0:|||&amp;cht=bhg&amp;chd=t:"+value1+"|"+value2+"|"+value3+"|"+value4+"|"+value5+"|"+value6+<?=$auxScriptValue?>"&amp;chdl="+label1+"|"+label2+"|"+label3+"|"+label4+"|"+label5+"|"+label6+<?=$auxScriptLabel?>"&amp;chco=ce9c52,d3cd83,fa5353,527bce,52c6ce,00886d<?=$auxScriptColor?>&amp;chbh=<?=(TWILIO_APP_ENABLED == "on" ? "15" : "25")?>' alt='Report Chart'/>";
        }
