<?
	...
	if (PAYFLOWPAYMENT_FEATURE == "on") {
		...
		if (!PAYFLOW_LOGIN || !PAYFLOW_PARTNER) {
			echo "<p class=\"errorMessage\">".system_showText(LANG_PAYFLOW_NO_AVAILABLE)." <a href=\"".DEFAULT_URL."/members/help.php\" class=\"billing-contact\">".system_showText(LANG_LABEL_ADMINISTRATOR)."</a>.</p>";
		} else {
			...
			<form name="payflowform" target="_self" action="<?=PAYFLOW_POST_URL?>" method="post">
				...
				<? if ($payment_process == "signup") { ?>
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						<tr>
							<td><p class="standardButton paymentButton"><a href="javascript:void(0);" id="payflowbutton" onclick="submitOrder();"><?=system_highlightWords(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))?></a></p></td>
						</tr>
					</table>
				<? } else { ?>
