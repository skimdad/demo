<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /includes/forms/form_article_seocenter.php
	# ----------------------------------------------------------------------------------------------------
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		for (j=0;j<arrLangNumbers.length;j++) {
			i = arrLangNumbers[j];
			var field_name = 'seo_abstract'+i;
			var count_field_name = 'seo_abstract_remLen'+i;
...
<table cellpadding="0" cellspacing="0" border="0" class="standard-table">
	...
	<tr>
		<td colspan="2" class="standard-tableContent">
			<?=system_showText(LANG_MSG_FRIENDLY_URL1)?><br /><br /><strong><?=system_showText(LANG_LABEL_FOR_EXAMPLE)?>:</strong><br /><br /><?=system_showText(LANG_MSG_FRIENDLY_URL2)?><br />"<?=ARTICLE_DEFAULT_URL?>/john-auto-repair.html"
		</td>
	</tr>
	<tr>
...
<table cellpadding="0" cellspacing="0" border="0" class="standard-table noMargin">
	...
	?>
		<tr id="description_<?=$i?>" <?=($k == 0) ? "style=\"display:table-row;\"" : "style=\"display:none;\""?> >
			<td colspan="2">
				<textarea id="seo_abstract<?=$labelsuffix;?>" name="seo_abstract<?=$labelsuffix;?>" rows="5" cols="1" ><?=${"seo_abstract".$labelsuffix};?></textarea>
