<?
	...
	if (AUTHORIZEPAYMENT_FEATURE == "on") {
		...
		if (!AUTHORIZE_LOGIN || !AUTHORIZE_TXNKEY) {
			echo "<p class=\"errorMessage\">".system_showText(LANG_AUTHORIZE_NO_AVAILABLE)." <a href=\"".DEFAULT_URL."/members/help.php\" class=\"billing-contact\">".system_showText(LANG_LABEL_ADMINISTRATOR)."</a>.</p>";
			...
			if (!$stoppayment) {
				...
				<form name="authorizeform" target="_self" action="<?=DEFAULT_URL?>/members/<?=$payment_process?>/processpayment.php?payment_method=<?=$payment_method?>" method="post">
