<?
	...
	if (PAYPALAPIPAYMENT_FEATURE == "on") {
		...
		if (!PAYPALAPI_USERNAME || !PAYPALAPI_PASSWORD || !PAYPALAPI_SIGNATURE) {
			echo "<p class=\"errorMessage\">".system_showText(LANG_PAYPALAPI_NO_AVAILABLE)." <a href=\"".DEFAULT_URL."/members/help.php\" class=\"billing-contact\">".system_showText(LANG_LABEL_ADMINISTRATOR)."</a>.</p>";
		} else {
			...
			<form name="paypalapiform" target="_self" action="<?=DEFAULT_URL?>/members/<?=$payment_process?>/processpayment.php?payment_method=<?=$payment_method?>" method="post">
				...
				<table align="center" width="95%" cellpadding="2" cellspacing="2" class="standard-tableTOPBLUE">
					...
					<tr>
						<th colspan="2" style="text-align:center"><?=system_showText(LANG_LABEL_CUSTOMER_INFO);?></th>
					</tr>
					<tr>
					...
					<tr>
						<td>* <?=string_ucwords(system_showText(LANG_LABEL_ZIP));?>:</td>
				...
				<? if ($payment_process == "signup") { ?>
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						<tr>
							<td><p class="standardButton paymentButton"><a href="javascript:void(0);" id="paypalapibutton" onclick="submitOrder();"><?=system_highlightWords(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))?></a></p></td>
						</tr>
					</table>
				<? } else { ?>
