<?
	...
	if (TWOCHECKOUTPAYMENT_FEATURE == "on") {
		...
		if (!TWOCHECKOUT_LOGIN) {
			echo "<p class=\"errorMessage\">".system_showText(LANG_TWOCHECKOUT_NO_AVAILABLE)." <a href=\"".DEFAULT_URL."/members/help.php\" class=\"billing-contact\">".system_showText(LANG_LABEL_ADMINISTRATOR)."</a>.</p>";
		} else {
			...
			<form name="twocheckoutform" target="_self" action="<?=TWOCHECKOUT_POST_URL?>" method="post">
				...
				<div style="display: none;">
					...
					<input type="hidden" name="x_Receipt_Link_URL" value="<?=DEFAULT_URL?>/members/<?=$payment_process?>/processpayment.php?payment_method=<?=$payment_method?>" />
