<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /includes/forms/form_listingsettings.php
	# ----------------------------------------------------------------------------------------------------
...
<div class="header-form"><?=string_ucwords(system_showText(LANG_SITEMGR_LABEL_CHANGE_LISTING_STATUS))?> - <?=$listingObj->getString("title")?></div>

<? if ($message_listingsettings) { ?>
	<div id="warning" class="errorMessage"><?=$message_listingsettings?></div>
<? } ?>

<? if ($listingObj->needToCheckOut()) { ?>
	<div id="warning" class="informationMessage"><?=string_ucwords(system_showText(LANG_SITEMGR_UNPAIDITEM))?></div>
<? } ?>
...
<table cellpadding="0" cellspacing="0" class="table-form table-form-settings table-form-margin">
	...
	<? if ($listingObj->hasRenewalDate()) { ?>
		<tr class="tr-form">
			...
			<td align="left" class="td-form">
				...
				$listingObj->renewal_date = $listingObj->getNextRenewalDate();
				$renewal_date = $listingObj->getDate("renewal_date");
				?>
				&nbsp;<a href="javascript:void(0);" title="<?=system_showText(LANG_SITEMGR_RENEWALDATE_AUTOFILL2)?>" onclick="document.forms['listing_setting'].elements['renewal_date'].value='<?=$renewal_date; ?>'"><?=system_showText(LANG_SITEMGR_RENEWALDATE_AUTOFILL)?></a>
	...
	<? if ($listingObj->getString("account_id")) { ?>
