<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /includes/forms/form_searchlisting.php
	# ----------------------------------------------------------------------------------------------------
...
<? if ($message_searchlisting) { ?>
	<div id="warning" class="errorMessage">
		<?=$message_searchlisting?>
	</div>
<? } ?>
...
<? //////////////////////////////////////////////////////////////////////////////////////////////////// ?>
	...
	<table border="0" cellpadding="2" cellspacing="0" class="standard-table noMargin">
        ...
        <tr>
			<th>&nbsp;</th>
			<td><input type="checkbox" value="1" name="search_backlink" <?=($search_backlink==1) ? "checked" : "";?> class="inputAlign" /><?=system_showText(LANG_LABEL_BACKLINK)?></td>
		</tr>
        <? } ?>
		<tr>
			<th><?=string_ucwords(system_showText(LANG_LABEL_SEARCHKEYWORD))?>:</th>
			<td><input type="text" name="search_title" value="<?=$search_title?>" class="input-form-searchlisting" /></td>
		</tr>

		<? if (!$user) { ?>
			<tr>
				<th><?=string_ucwords(system_showText(LANG_SITEMGR_SETTINGS_LEVELS_LEVEL))?>:</th>
				<td>
					<table style="width: auto;">
						<tr>
							<?
							$level = new ListingLevel('', false, false, true);
		...
		<? if (LISTINGTEMPLATE_FEATURE == "on" && !USING_THEME_TEMPLATE) { ?>
			<tr>
				<th><?=string_ucwords(system_showText(LANG_SITEMGR_LISTING_LISTINGTEMPLATE))?>:</th>
				<td><?=$listingTemplateDropDown?></td>
			</tr>
		<? } ?>
		...
		<? if ( PAYMENTSYSTEM_FEATURE == 'on' ) { ?>
			...
			<tr>
				...
				<td><input type="text" name="search_discount" value="<?=$search_discount?>" maxlength="10" class="input-form-searchlisting" /></td>
			</tr>
	...
	<table border="0" cellpadding="2" cellspacing="0" class="standard-table">
		...
		<tr>
			...
			<td><input type="text" name="search_zipcode" value="<?=$search_zipcode?>" maxlength="10" class="input-form-searchlisting" /></td>
		</tr>
