<?
	...
	if (LINKPOINTPAYMENT_FEATURE == "on") {
		...
		if (!LINKPOINT_CONFIGFILE || !LINKPOINT_KEYFILE) {
			echo "<p class=\"errorMessage\">".system_showText(LANG_LINKPOINT_NO_AVAILABLE)." <a href=\"".DEFAULT_URL."/members/help.php\" class=\"billing-contact\">".system_showText(LANG_LABEL_ADMINISTRATOR)."</a>.</p>";
		} else {
			...
			if (!$stoppayment) {
				...
				<form name="linkpointform" target="_self" action="<?=DEFAULT_URL?>/members/<?=$payment_process?>/processpayment.php?payment_method=<?=$payment_method?>" method="post">
					...
					<table align="center" width="95%" cellpadding="2" cellspacing="2" class="standard-table" >
						...
						<tr>
							...
							<td>
								...
								?>
								<select name="state" title="State" onchange="this.form.state2.value='';" id="state">
								...
								<input type="text" name="state2" title="Other State code" size="2" maxlength="2" value="" onchange="if(this.value != '')this.form.state.options[0].selected = true;" id="state2" style="display:none" />
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
								...
								?>
								<select name="country" title="Country" onchange="this.form.state.options[0].selected = true; this.form.state2.value=''; checkState(this.value);">
					...
					<? if ($payment_process == "signup") { ?>
						<table width="100%" border="0" cellpadding="2" cellspacing="2">
							<tr>
								<td><p class="standardButton paymentButton"><a href="javascript:void(0);" id="linkpointbutton" onclick="submitOrder();"><?=system_highlightWords(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))?></a></p></td>
							</tr>
						</table>
					<? } else { ?>
