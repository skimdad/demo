<?
	...
	if (PAYPALPAYMENT_FEATURE == "on") {
		...
		if (!PAYPAL_ACCOUNT) {
			echo "<p class=\"errorMessage\">".system_showText(LANG_PAYPAL_NO_AVAILABLE)." <a href=\"".DEFAULT_URL."/members/help.php\" class=\"billing-contact\">".system_showText(LANG_LABEL_ADMINISTRATOR)."</a>.</p>";
			...
			if (!$stoppayment) {
				...
				//if (PAYPALRECURRING_FEATURE == "on") {
				//	$paypal_return = DEFAULT_URL."/members/".$payment_process."/processpaypalpayment.php";
				//} else {
					$paypal_return = DEFAULT_URL."/members/".$payment_process."/processpayment.php?payment_method=".$payment_method."";
				//}
				$paypal_cancel_return = DEFAULT_URL."/members/".$payment_process."/processpayment.php?payment_method=".$payment_method."&cancel=true";
				$paypal_notify_url = NON_LANG_URL."/members/billing/receipt.php";
				...
				<form name="paypalform" target="_self" action="https://<?=PAYPAL_URL?><?=PAYPAL_URL_FOLDER?>" method="post">
					...
					<? if ($payment_process == "signup") { ?>
						<table width="100%" border="0" cellpadding="2" cellspacing="2">
							<tr>
								<td><p class="standardButton paymentButton"><a href="javascript:void(0);" id="paypalbutton" onclick="submitOrder();"><?=system_highlightWords(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))?></a></p></td>
							</tr>
						</table>
					<? } else { ?>
