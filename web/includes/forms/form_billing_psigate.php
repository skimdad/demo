<?
	...
	if (PSIGATEPAYMENT_FEATURE == "on") {
		...
		if (!PSIGATE_STOREID || !PSIGATE_PASSPHRASE) {
			echo "<p class=\"errorMessage\">".system_showText(LANG_PSIGATE_NO_AVAILABLE)." <a href=\"".DEFAULT_URL."/members/help.php\" class=\"billing-contact\">".system_showText(LANG_LABEL_ADMINISTRATOR)."</a>.</p>";
		} else {
			...
			<form name="psigateform" target="_self" action="<?=DEFAULT_URL?>/members/<?=$payment_process?>/processpayment.php?payment_method=<?=$payment_method?>" method="post">
				...
				<? if ($payment_process == "signup") { ?>
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						<tr>
							<td><p class="standardButton paymentButton"><a href="javascript:void(0);" id="psigatebutton" onclick="submitOrder();"><?=system_highlightWords(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))?></a></p></td>
						</tr>
					</table>
				<? } else { ?>
