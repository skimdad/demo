<?
	...
	<div class="extendedContent membersExtendedContent">
		...
		if ($imageObj->imageExists()) {
			echo $imageObj->getTag(true, IMAGE_PACKAGE_FULL_WIDTH, IMAGE_PACKAGE_FULL_WIDTH, $packageObj->getString("title"));
		}

		?>

		<? if($packageObj->getStringLang(EDIR_LANGUAGE, "content", false)) { ?><div class="content-custom"><?=($packageObj->getStringLang(EDIR_LANGUAGE, "content", false))?></div><? } ?>

		<table class="packageTable">
			<tr>

				<th class="standardSubTitle" colspan="4"><?=$msg_packagetr?></th>
			...
			?>
					...
					?>
						<tr>
							<td class="packageOffer"><?=$aux_package_item_desc ? $aux_package_item_desc : LANG_CUSTOM_OPTION?></td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
		...
		if(string_strpos($_SERVER["PHP_SELF"], "signup") !== false){
			...
			if ($checkout == "true"){
				$action = DEFAULT_URL."/members/$item_type/$item_type.php?id=$item_id&process=signup";
			} else {
				if ($payment_method == "invoice"){
					$action = DEFAULT_URL."/members/signup/invoice.php";
				} else {
					$action = DEFAULT_URL."/members/signup/payment.php?payment_method=".$payment_method;
				}
			}
		}
		?>
		...
		<div class="baseButtons">
			...
			if(string_strpos($_SERVER["PHP_SELF"], "signup") !== false){
				...
				<p style="float:right" class="standardButton checkoutButton">
					<button type="submit"><?=system_showText(LANG_BUTTON_YES_CONTINUE)?></button>
				</p>
				<?
			}else{
				?>
				...
				<p style="float:right" class="standardButton checkoutButton">
					<button type="button" onclick="addPackage();"><?=system_showText(LANG_BUTTON_YES_CONTINUE)?></button>
				</p>
				<?
			}
			?>
