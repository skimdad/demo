<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /deal/detail_reviews.php
	# ----------------------------------------------------------------------------------------------------
	
	if (!$hideDetail && $review_enabled == "on" && $commenting_edir){
		...
		<h2>
			...
			<? } ?>
			<a class="view-more <?=$class?> rate-it" href="<?=$linkReviewFormPopup?>" <?=$linkStyle?>>
				<? if($review_amount > 0){ 
		...
		<div class="featured featured-review">
			<? if($review_amount == 0){ ?>
				<a class="<?=$class?> rate-it-first" href="<?=$linkReviewFormPopup?>" <?=$linkStyle?>><?=system_showText(LANG_REVIEWBETHEFIRST);?></a>
			<? }else{
				echo $deal_review;
			} ?>
		</div>
...
<? } ?>
	...
	if (!$hideDetail && $commenting_fb){ ?>
		...
		if ($user) {
			...
			$detailLink = NON_LANG_URL."/".PROMOTION_FEATURE_FOLDER."/facebook_comment.php?id=".$promotion->getNumber("id");
			...
			<script language="javascript" type="text/javascript">
				...
				document.write('<div class="fb-comments" data-href="<?=$detailLink?>" data-num-posts="<?=$commenting_fb_number_comments?>" data-width="245"></div>');
				//]]>
			</script>
