<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /coupon/relatedcategories.php
	# ----------------------------------------------------------------------------------------------------
	...
	if ($keyword) {
		...
		if (LISTINGCATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			$sql = "SELECT * FROM ListingCategory WHERE category_id = 0 AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND enabled = 'y' AND ".$where_clause."";
		} else {
			$sql = "SELECT * FROM ListingCategory WHERE lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND enabled = 'y' AND ".$where_clause."";
		}
	...
	if ($related_categories) {
		...
		$category_tree = system_generateCategoryTree($related_categories, $arr_full_path, "promotion", $user, EDIR_LANGUAGE);
	}
