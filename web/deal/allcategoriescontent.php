<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /deal/allcategoriescontent.php
	# ----------------------------------------------------------------------------------------------------
			...
			for($i=0;$i<count($xml_categories->info);$i++){
				...
				if($categories){
					...
					if (MODREWRITE_FEATURE == "on") {
						echo "<a href=\"".PROMOTION_DEFAULT_URL."/guide/".$categories["friendly_url".$langIndex]."\" title = \"".string_htmlentities($categories["title".$langIndex])."\">";
					} else {
						echo "<a href=\"".PROMOTION_DEFAULT_URL."/results.php?category_id=".$categories["id"]."\" title = \"".string_htmlentities($categories["title".$langIndex])."\">";
					}
