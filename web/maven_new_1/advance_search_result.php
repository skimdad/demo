...
if (!$search_lock) {
    ...
    $aux_items_per_page = ($_COOKIE["listing_results_per_page"] ? $_COOKIE["listing_results_per_page"] : 2);
    ...
    $paging_url = MOBILE_DEFAULT_URL . "/" . $selfpage;

    // the following code is modified by Debiprasad on 26 July 2012 for getting all bronze listings
    if (!($_GET['id'] || $_GET['keyword'])) {
        $get_requests = $_GET;
    ...
    $aux_CategoryModuleURL = MOBILE_DEFAULT_URL;
...
if (!$bronze_listings && !$letterb)
    $showLetterBronze = false;


# ----------------------------------------------------------------------------------------------------
# BODY
# ----------------------------------------------------------------------------------------------------
include('./listing_result_advance.php');
//echo 'hgyjuytyuiytuyt';
# ----------------------------------------------------------------------------------------------------
# FOOTER
# ----------------------------------------------------------------------------------------------------
$banner_section = "listing";
