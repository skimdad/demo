...
if ($listingObj) {
    ...
    if ($location_map) {
        ?>
        <img style="display:block;margin: 3px auto" src="http://maps.google.com/maps/api/staticmap?center=<?= $location_map ?>&zoom=15&size=280x150&maptype=roadmap&mobile=true&markers=icon:<?= $icon ?>|<?= $location_map ?>&sensor=false" />
    <? } ?>

    <div class="itemView listingView innerDetails">
        <h1 class="itemTitle">
            <?= $listing["title"] ?>
        </h1>
            ...
            <address>
                ...
                if ($listing["location_3"] || $listing["location_4"] || $listing["zip_code"]) {
                    ...
                    <span>
                        ...
                        if ($listing["location_4"])
                            echo $string_location4;
                        if ($listing["location_3"] && $listing["location_4"])
                            echo ", ";
                        if ($listing["location_3"])
                            echo $string_location3;
                        if ($listing["location_3"] || $listing["location_4"])
                            echo " ";
                        if ($listing["zip_code"])
