<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /event/relatedcategories.php
	# ----------------------------------------------------------------------------------------------------
	...
	if ($keyword) {
		...
		if (EVENTCATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			$sql = "SELECT * FROM EventCategory WHERE category_id = 0 AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND enabled = 'y' AND ".$where_clause."";
		} else {
			$sql = "SELECT * FROM EventCategory WHERE lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND enabled = 'y' AND ".$where_clause."";
		}
		...
		while ($row = mysql_fetch_assoc($rs)) {
			$related_categories[] = new EventCategory($row["id"]);
		}
	...
	if ($related_categories) {
		...
		$category_tree = system_generateCategoryTree($related_categories, $arr_full_path, "event", $user, EDIR_LANGUAGE);
	}
