<?
	...
	if ($_GET["url_full"])
		$_GET["url_full"] = urldecode(str_replace(EDIRECTORY_FOLDER.(REDIRECT_EDIR_LANGUAGE ? "/".EDIR_LANGUAGEABBREVIATION : "")."/".EVENT_FEATURE_FOLDER."/results.php?url_full=","",$_SERVER["REQUEST_URI"]));
	...
	if ($_GET["url_full"] && (string_strpos($_GET["url_full"], "search") !== false || string_strpos($_GET["url_full"], "guide") !== false || string_strpos($_GET["url_full"], "location") !== false)) {
		...
		$url = string_replace_once(EDIRECTORY_FOLDER.(REDIRECT_EDIR_LANGUAGE ? "/".EDIR_LANGUAGEABBREVIATION : "")."/".EVENT_FEATURE_FOLDER."/", "", $_GET["url_full"]);
		$parts = explode("/", $url);
		...
		if (string_strpos($_GET["url_full"],"guide") !== false ){
		...
		} else if (string_strpos($_GET["url_full"],"month") !== false || string_strpos($_GET["url_full"],"this_date") !== false){
			
			for ($i=0; $i<count($parts); $i++){
					switch($parts[$i]) {
					case 'this_date': $_GET["this_date"] = $parts[$i+1];
									break;
					case 'month': $_GET["month"] = $parts[$i+1];
									break;
					case 'search_by_day': $_GET["search_by_day"] = "true";
									break;
			...
			if (!$parts[1] && !$parts[3]) {
				header("Location: ".EVENT_DEFAULT_URL."/search/empty/where/empty");
				exit;
			}
	...
	##################################################
	if ($_GET["category1"]) {
		$sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber("0")." AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND friendly_url".$langIndex." = ".db_formatString($_GET["category1"])." AND enabled = 'y' LIMIT 1";
	...
	if ($_GET["category2"] && $_GET["category_id"] && !$failure) { 
		$sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($_GET["category_id"])." AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND friendly_url".$langIndex." = ".db_formatString($_GET["category2"])." AND enabled = 'y' LIMIT 1";
		$result = $dbObj->query($sql);
		$aux = mysql_fetch_assoc($result);
		$_GET["category_id"] = $aux["id"];
		if (!$_GET["category_id"]) $failure = true;
    ...
    if ($_GET["category3"] && $_GET["category_id"] && !$failure) { 
        $sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($_GET["category_id"])." AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND friendly_url".$langIndex." = ".db_formatString($_GET["category3"])." AND enabled = 'y' LIMIT 1";
        $result = $dbObj->query($sql);
        $aux = mysql_fetch_assoc($result);
        $_GET["category_id"] = $aux["id"];
        if (!$_GET["category_id"]) $failure = true;
    ...
    if ($_GET["category4"] && $_GET["category_id"] && !$failure) {
        $sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($_GET["category_id"])." AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND friendly_url".$langIndex." = ".db_formatString($_GET["category4"])." AND enabled = 'y' LIMIT 1";
        $result = $dbObj->query($sql);
        $aux = mysql_fetch_assoc($result);
        $_GET["category_id"] = $aux["id"];
        if (!$_GET["category_id"]) $failure = true;
    ...
    if ($_GET["category5"] && $_GET["category_id"] && !$failure) {
        $sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($_GET["category_id"])." AND lang LIKE ".db_formatString("%".EDIR_LANGUAGE."%")." AND friendly_url".$langIndex." = ".db_formatString($_GET["category5"])." AND enabled = 'y' LIMIT 1";
        $result = $dbObj->query($sql);
        $aux = mysql_fetch_assoc($result);
        $_GET["category_id"] = $aux["id"];
        if (!$_GET["category_id"]) $failure = true;
