<?php
	...
	class Image extends Handle {
		 ...
		 */
		function getTag($resize = false, $maxWidth = 0, $maxHeight = 0, $title="", $force_resize = false, $alternative_text = false) {
			
			if (RESIZE_IMAGES_UPGRADE == "off"){
				$force_resize = false;
			}
			
			if(FORCE_ANTIALIASED_IMAGES == "off" || string_strtolower($this->type) == "gif") {
				return $this->_getTag($resize, $maxWidth, $maxHeight, $title, $force_resize, $alternative_text);
			} else {
				return $this->getAntialiasedTag(true, $maxWidth, $maxHeight, $title, $force_resize, false, $alternative_text);
			}
		}
		 ...
		 * @return varchar htmlcode
		 */
		function _getTag($resize = false, $maxWidth = 0, $maxHeight = 0, $title="", $force_resize = false, $alternative_text = false){
			$type = string_strtolower($this->type);
			...
			$title = $aux_image_seo;
			if ($force_resize) {
				$return = "<img width=\"".$maxWidth."\" height=\"".$maxHeight."\" src=\"".$this->IMAGE_URL."/".$this->prefix."photo_".$this->id.".".$type."\" ".$title." border=\"0\" />";
			} elseif($resize) {
				image_getNewDimension($maxWidth, $maxHeight, $this->width, $this->height, $newWidth, $newHeight);
				$return = "<img width=\"".(int)$newWidth."\" height=\"".(int)$newHeight."\" src=\"".$this->IMAGE_URL."/".$this->prefix."photo_".$this->id.".".$type."\" ".$title." border=\"0\" ".(RESIZE_IMAGES_UPGRADE == "off"? "style=\"display:inline\"" : "")." />";
			} else {
				$return = "<img src=\"".$this->IMAGE_URL."/".$this->prefix."photo_".$this->id.".".$type."\" ".$title." border=\"0\" />";
			}
			return $return;
		}
		 ...
		 * @return varchar htmlcode
		 */
		function getAntialiasedTag($resize = false, $maxWidth, $maxHeight, $title = "", $distort = false, $img_path = false, $alternative_text = false) {
			...
			return "<img alt=\"".string_htmlentities($alternative_text)."\" title=\"".string_htmlentities($title)."\" src=\"".DEFAULT_URL."/image_resizer.php?img=".$file."&newWidth=".(int)$maxWidth."&newHeight=".(int)$maxHeight."\" width=\"".(int)$maxWidth."\" height=\"".(int)$maxHeight."\" border=\"0\" />";
		}
