<?
	...
	class BannerLevel {
		...
		function BannerLevel($lang = EDIR_DEFAULT_LANGUAGE, $listAll = false, $domain_id = false) {
		...
		function getContent($value) {
			...
			unset($dbMain);
			$sql   = "SELECT content FROM BannerLevel_Lang WHERE value='$value' AND lang='$this->lang' AND theme = ".db_formatString(EDIR_THEME ? EDIR_THEME : "default")."";
			$result = $dbObj->query($sql);
        ...
        function updateValues($name, $active, $levelValue){
            ...
            $sql = "UPDATE BannerLevel SET displayName = '".$name."', active = '".$active."' WHERE value = ".$levelValue." AND theme = '".(EDIR_THEME ? EDIR_THEME : "default")."'";
            $dbObj->query($sql);
