<?php
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /classes/class_imageResizer.php
	# ----------------------------------------------------------------------------------------------------
	...
	**/
    ...
    Class ImageResizer
    {
        ...
        {
		//htmlencde("this->imgWidth = ".$this->imgWidth." percent = ".$percent);
		//htmlencde("this->imgWidth = ".$this->imgWidth." percent = ".$percent,1);
        ...
        {
		htmlencde("this->imgWidth = ".$this->imgWidth." percent = ".$percent,0);
		htmlencde("this->imgWidth = ".$this->imgWidth." percent = ".$percent,1);
        ...
        function _resize($width,$height,$newfile=NULL)
        {
            ...
            if($newfile===HAR_AUTO_NAME)
            {
                if(@preg_match("/\..*+$/",@basename($this->imgFile),$matches))
                       $newfile=@substr_replace($this->imgFile,"_har",-@string_strlen($matches[0]),0);            
            }
