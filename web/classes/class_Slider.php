<?
	...
	class Slider extends Handle {
		 ...
		 * @param integer $var
		 */
		function Slider($var='',$pslider_kind='HOME',$pslider_loc1=0,$pslider_loc3="",$pslider_loc4="",$pslider_cat=0, $domain_id = false) {
		...
		function makeFromRow($row='') {
				...
				$this->slider_kind				= ($row["slider_kind"])				? $row["slider_kind"]			: ($this->slider_kind			? $this->slider_kind			: "");
				$this->slider_cat				= ($row["slider_cat"])				? $row["slider_cat"]			: ($this->slider_cat			? $this->slider_cat			: 0);
				$this->slider_loc1				= ($row["slider_loc1"])				? $row["slider_loc1"]			: ($this->slider_loc1			? $this->slider_loc1			: 0);				
				$this->slider_loc3				= ($row["slider_loc3"])				? $row["slider_loc3"]			: ($this->slider_loc3			? $this->slider_loc3			: "");								
				$this->slider_loc4				= ($row["slider_loc4"])				? $row["slider_loc4"]			: ($this->slider_loc4			? $this->slider_loc4			: "");								
		}
		...
		function Save() {
			...
			if ($this->id) {
				...
				$sql  = "UPDATE Slider SET"
					...
					. " slider_kind         = $this->slider_kind,"
					. " slider_cat         = $this->slider_cat,"
					. " slider_loc1         = $this->slider_loc1,"					
					. " slider_loc3         = $this->slider_loc3,"					
					. " slider_loc4         = $this->slider_loc4"										
					. " WHERE id            = $this->id";
		...
		function getAllSliderItems(){
			...
			if ($_SESSION["kind"] != "" && $_SESSION["state"] != "" && $_SESSION["city"] != "") {
				$where_clause = " AND slider_kind = '".$_SESSION["kind"]."'"; 
				//$where_clause .= " AND slider_cat = ".$_SESSION["catid"];
				//$where_clause .= " AND slider_loc1 = ".$_SESSION["subcatid"];
				$where_clause .= " AND slider_loc3 = '".$_SESSION["state"]."'";
				$where_clause .= " AND slider_loc4 = '".$_SESSION["city"]."'";
				
				$sql = "SELECT * FROM Slider WHERE 1 ".$where_clause." ORDER BY slide_order LIMIT ".TOTAL_SLIDER_ITEMS;
			} else {
				$sql = "SELECT * FROM Slider WHERE id < 11 ORDER BY slide_order LIMIT ".TOTAL_SLIDER_ITEMS;
			}
