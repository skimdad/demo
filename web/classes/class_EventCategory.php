	...
	class EventCategory extends Handle {
		...
		function getFullPath($sitemgr = false) {
			...
			if (count($arrLangs) > 1) {
				...
				$fields = "`id`, `category_id`, `lang`, `active_event`, `featured`, `enabled`, `friendly_url".$langIndex."`, `title".$langIndex."` AS $fieldAux";
			}
		...
		function updateFullTextItems() {
			...
			if ($this->id) {
				...
				$sql   		   = "SELECT id FROM Event WHERE $whereCategory";
				$result        = $dbObj->query($sql);
