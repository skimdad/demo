		...
		function getCodeOfPagination($array_pages, $aux_page_url, $aux_url = false, $force_total = false){
			...
			if($array_pages["show_pages"]){
				...
				if($array_pages["pages"]["previous"] != $this->aux_page_num){
					$array_links["previous"] = "<li><a href='".$aux_page_url.$array_pages["pages"]["previous"].($aux_url ? $aux_url : "") . ($hashid ? $hashid : '') . "'>".system_showText(LANG_PAGING_PREVIOUSPAGEMOBILE)."</a></li>";
				}
				
				if(!in_array(1,$array_pages["pages"]["pages"])){
					$array_links["first"] = "				
						<li><a href='".$aux_page_url."1".($aux_url ? $aux_url : "") . ($hashid ? $hashid : '') . "'>1</a></li>
						<li>&nbsp;....&nbsp;</li>";		
				}
				...
				for($i=0;$i<count($array_pages["pages"]["pages"]);$i++){
					$aux_page_code .= "
					<li><a".($array_pages["pages"]["pages"][$i] == $this->aux_page_num ? " class=\"active\" " : "")." href='".$aux_page_url.$array_pages["pages"]["pages"][$i].($aux_url ? $aux_url : "") . ($hashid ? $hashid : '') . "'>".$array_pages["pages"]["pages"][$i]."</a></li>";					
				}
				...
				if(!in_array($array_pages["pages"]["last"],$array_pages["pages"]["pages"])){
					$array_links["last"] = "
					<li>&nbsp;....&nbsp;</li>
					<li><a href='".$aux_page_url.$array_pages["pages"]["last"].($aux_url ? $aux_url : "") . ($hashid ? $hashid : '') . "'>".$array_pages["pages"]["last"]."</a></li>";					
				}
				if($array_pages["pages"]["next"] != $this->aux_page_num){
					$array_links["next"] = "						
					<li><a href='".$aux_page_url.$array_pages["pages"]["next"].($aux_url ? $aux_url : "") . ($hashid ? $hashid : '') . "'>".system_showText(LANG_PAGING_NEXTPAGEMOBILE)."</a></li>";
					
				}				
			}
			//echo "<pre>";print_r($array_links);echo "</pre>";
			return $array_links;
