<?
	...
	class Handle {
		...
		function getString($field, $special_chars=true, $length = 0, $extraChar = "...") {
			$value = $this->$field;
			if (!is_string($value)) return $value;
			$value = ($length > 0) ? system_showTruncatedText($value, $length, $extraChar, true) : $value;
			$value = ($special_chars) ? htmlspecialchars($value, ENT_NOQUOTES) : $value ;
			return $value;
		}

		function getStringLang($lang=EDIR_DEFAULT_LANGUAGE, $field, $special_chars=true, $length = 0, $extraChar = "...") {
			$labelsuffix = language_getIndex($lang);
			return $this->getString($field.$labelsuffix, $special_chars, $length, $extraChar);
		}
		...
		function extract() {
			...
			for ($i=0; $i < count($vars); $i++) {
				...
				if (count($key["value"]) > 1) {
					$value = $key["value"];
				} else if (preg_match($regexp_date, $key["value"])){
					$value = $this->getDate("{$key["key"]}");
					if ($value == "00/00/0000") unset($value);
				} elseif (preg_match($regexp_decimal, $key["value"])) {
					$value = $key["value"];
				} else {
					$value = $key["value"];
				}
