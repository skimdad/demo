<?
	...
	class Location5 extends Handle {
		...
		function makeFromRow($row='') {
			...
			if ($row['location_4']) $this->location_4 = $row['location_4'];
			else if (!$this->location_4) $this->location_4 = 0;
		...
		function Save($updFullText = false) {
			...
			if ($this->id) {
				system_logLocationChanges($this->id, 5, $this->location_1, 1);
				system_logLocationChanges($this->id, 5, $this->location_2, 2);
				system_logLocationChanges($this->id, 5, $this->location_3, 3);
				system_logLocationChanges($this->id, 5, $this->location_4, 4);
				...
				$sql = "INSERT INTO Location_5"
					. " (location_4, location_3, location_2, location_1, name, abbreviation, friendly_url, seo_description, seo_keywords)"
					. " VALUES"
					. " ($this->location_4, $this->location_3, $this->location_2, $this->location_1, $this->name, $this->abbreviation, $this->friendly_url, $this->seo_description, $this->seo_keywords)";
				$dbObj->query($sql);
		...
		function Delete($updFullText = false) {
			...
			if ($this->id) {
				...
				while ($row = mysql_fetch_assoc($resDomain)) {
					...
					//Listing
					$sql = "UPDATE Listing SET location_5 = 0 WHERE location_5 = $this->id";
					...
					//Listing_Summary
					$sql = "UPDATE Listing_Summary SET location_5 = 0,
													   location_5_title = '',
		...
		function retrieveFeatureds($_locations, $default=false, $last_default_level=false, $last_default_id=false) {
			...
			$sql .= " WHERE Location_5.".($default==false?"id IN (".$locationFeatObj->getFeatureds(SELECTED_DOMAIN_ID, 5).")":"id = $default");
		...
		function retrieveAllLocation(){
			$dbObj = db_getDBObject(DEFAULT_DB,true);
			$sql = "SELECT * FROM Location_5 ORDER BY name";
			$result = $dbObj->query($sql);
		...
		function retrieveLocationByLocation($father_level){
			...
			$dbObj = db_getDBObject(DEFAULT_DB,true);
			$sql = "SELECT * FROM Location_5 WHERE location_".$father_level." = ".$father_level_value." ORDER BY name";
			$result = $dbObj->query($sql);
			while($row = mysql_fetch_assoc($result)) $data[] = $row;
			if($data) return $data; else return false;
		}
		...
		function isValidFriendlyUrl($father_level, &$error_message) {
			...
			$dbObj = db_getDBObject(DEFAULT_DB,true);
			$sql = "SELECT friendly_url FROM Location_5 WHERE friendly_url = '".$this->getString("friendly_url")."'";
			if ($father_level !== false) {
				$father_level_value = "location_".$father_level;
				if ($this->$father_level_value) {
					$father_level_value = $this->$father_level_value;
