...
class Promotion extends Handle {
    ...
    function getPromotionByListing($listing_id) {
    ...
    function makeFromRow($row = '') {
        ...
        else if (!$this->image_id)
            $this->image_id = 0;
        if ($row["thumb_id"])
            $this->thumb_id = $row["thumb_id"];
        else if (!$this->thumb_id)
            $this->thumb_id = 0;
        if ($row["updated"])
     ...
     * <code>
     * 		//Using this in forms or other pages.
     * 		$promotionObj->getDealsFromUserAndListing($account_id, $listing_id);
     * <br /><br />
     * 		//Using this in Promotion() class.
     * 		$this->getDealsFromUser($friendly_url);
     * </code>
     ...
     * @param boolean $account_id
     */
    function getDealsFromUserAndListing($account_id , $listing_id) {
        if (!$account_id)
            return false;
        $sql = "SELECT Promotion_Redeem.* FROM Promotion_Redeem INNER JOIN Promotion ON Promotion_Redeem.promotion_id = Promotion.id WHERE Promotion_Redeem.account_id = $account_id  and Promotion.listing_id = $listing_id ORDER BY datetime DESC";
        $dbMain = db_getDBObject(DEFAULT_DB, true);
        $dbDomain = db_getDBObjectByDomainID(SELECTED_DOMAIN_ID, $dbMain);
        $result = $dbDomain->query($sql);
        $res = false;
