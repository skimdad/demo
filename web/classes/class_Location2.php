<?
	...
	class Location2 extends Handle {
		...
		function Save($father_level = false, $updFullText = false) {
			...
			if ($this->id) {
				...
				$sql = "INSERT INTO Location_2"
					...
					. " VALUES"
					. " ($this->location_1, $this->name, $this->abbreviation, $this->friendly_url, $this->seo_description, $this->seo_keywords)";
				$dbObj->query($sql);
		...
		function Delete($updFullText = false) {
			...
			if ($this->id) {
				...
				while ($row = mysql_fetch_assoc($resDomain)) {
					...
					//Listing
					$sql = "UPDATE Listing SET location_2 = 0, location_3 = 0, location_4 = 0, location_5 = 0 WHERE location_2 = $this->id";
					$result = $dbObj->query($sql);
                    
                    //Promotion
					$sql = "UPDATE Promotion SET    listing_location2 = 0, 
                                                    listing_location3 = 0, 
                                                    listing_location4 = 0, 
                                                    listing_location5 = 0 
					...
					//Listing_Summary
					$sql = "UPDATE Listing_Summary SET location_2 = 0,
													   location_2_title = '',
					...
					//Event
					$sql = "UPDATE Event SET location_2 = 0, location_3 = 0, location_4 = 0, location_5 = 0 WHERE location_2 = $this->id";
					$result = $dbObj->query($sql);
					//Classified
					$sql = "UPDATE Classified SET location_2 = 0, location_3 = 0, location_4 = 0, location_5 = 0 WHERE location_2 = $this->id";
					$result = $dbObj->query($sql);
				}
				unset($rowDomain);
		...
		function retrieveFeatureds($_locations, $default=false, $last_default_level=false, $last_default_id=false) {
			...
			$sql .= " WHERE Location_2.".($default==false?"id IN (".$locationFeatObj->getFeatureds(SELECTED_DOMAIN_ID, 2).")":"id = $default");
		...
		function retrieveAllLocation(){
			$dbObj = db_getDBObject(DEFAULT_DB,true);
			$sql = "SELECT * FROM Location_2 ORDER BY name";
			$result = $dbObj->query($sql);
		...
		function retrieveLocationByLocation($father_level){
			...
			$dbObj = db_getDBObject(DEFAULT_DB,true);
			$sql = "SELECT * FROM Location_2 WHERE location_".$father_level." = ".$father_level_value." ORDER BY name";
			$result = $dbObj->query($sql);
			while($row = mysql_fetch_assoc($result)) $data[] = $row;
			if($data) return $data; else return false;
		}
		...
		function isValidFriendlyUrl($father_level, &$error_message) {
			...
			$dbObj = db_getDBObject(DEFAULT_DB,true);
			$sql = "SELECT friendly_url FROM Location_2 WHERE friendly_url = '".$this->getString("friendly_url")."'";
			if ($father_level !== false) {
				$father_level_value = "location_".$father_level;
				if ($this->$father_level_value) {
					$father_level_value = $this->$father_level_value;
