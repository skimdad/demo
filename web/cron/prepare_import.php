...
<?
	...
	$sqlIL = "SELECT `id`, `date`, `time`, `filename`, `linesadded`, `phisicalname`, `status`, `action`, `progress`, `totallines`, `history`, `update_itens`, `mysqlerror` FROM `ImportLog` WHERE `status` = 'P' AND `action` = 'NC' AND type = 'listing' AND `mysqlerror` = '' ORDER BY `date` AND `time`";
			...
			while ($line = fgetcsv($handle, 16384, $delimiter)) {
				...
				if (!$tempError && !$import_stop) {
					...
					if (strlen($sqlBlock) == 0 && strlen($sqlLine) == 0) {
						$sqlLine = "INSERT INTO `ImportTemporary` (`id`, `import_log_id`, `file_line_number`, `account_username`, `account_password`, `account_first_name`, `account_last_name`, `account_company`, `account_address`, `account_address2`, `account_country`, `account_state`, `account_city`, `account_zip`, `account_phone`, `account_fax`, `account_email`, `account_url`, `legacy_id`, `listing_id`, `listing_title`, `listing_email`, `listing_url`, `listing_address`, `listing_address2`, `listing_location1`, `listing_location1_abbreviation`, `listing_location2`, `listing_location2_abbreviation`, `listing_location3`, `listing_location3_abbreviation`, `listing_location4`, `listing_location4_abbreviation`, `listing_location5`, `listing_location5_abbreviation`, `listing_zip`, `listing_latitude`, `listing_longitude`, `listing_phone`, `listing_fax`, `listing_description`, `listing_long_description`, `listing_keyword`, `listing_updated_date`, `listing_renewal_date`, `listing_status`, `listing_level`, `listing_category_1`, `listing_category_2`, `listing_category_3`, `listing_category_4`, `listing_category_5`, `listing_template`) VALUES ";
					}
				...
				if (count($line) > count($sample_header)) {
					...
					for ($i=0; $i<count($sample_header); $i++) {
						...
						if ($i == 0 || $i == 1 || $i == 17 || $i == 22 || $i == 24 || $i == 26 || $i == 28 || $i == 30 || $i == 33 || $i == 34 || $i == 39 || $i == 40 || $i == 41) {
				...
				<html>
					...
					<body>
						<div class=\"email_style_settings\">
							...
							$sitemgr_msg .= "<br /><a href=\"http://".$default_url."/sitemgr/import/importlog.php?log_id=".$importlogObj->getNumber("id")."\" target=\"_blank\">http://".$default_url."/sitemgr/import/importlog.php?log_id=".$importlog_id."</a><br /><br />
						</div>
					</body>
				</html>";
				...
				<html>
					...
					<body>
						<div class=\"email_style_settings\">
							...
							$sitemgr_msg .= "<br /><a href=\"http://".$default_url."/sitemgr/import/importlog.php?log_id=".$importlogObj->getNumber("id")."&preview=true\" target=\"_blank\">http://".$default_url."/sitemgr/import/importlog.php?log_id=".$importlogObj->getNumber("id")."&preview=true</a><br /><br />
						</div>
					</body>
				</html>";
	...
	if (!$import_stop) {
		print "Import Correction Process on Domain ".SELECTED_DOMAIN_ID." - ".date("Y-m-d H:i:s")." - ".string_ucwords(LISTING_FEATURE_NAME_PLURAL).": ".$totallines." - ".round($time, 2)." seconds.\n";
