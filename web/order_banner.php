<?
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($boolean_seckey && $validate_account && $validate_contact && $validate_banner && $validate_discount) {
				...
				<html>
					...
					<body>
						<div class=\"email_style_settings\">
							...
							$sitemgr_msg .= "Banner:<br /><br />";
							$sitemgr_msg .= "<b>Caption: </b>".$banner->getString("caption".EDIR_DEFAULT_LANGUAGENUMBER)."<br />";
							$sitemgr_msg .= "<br /><a href=\"".DEFAULT_URL."/sitemgr/".BANNER_FEATURE_FOLDER."/view.php?id=".$banner->getNumber("id")."\" target=\"_blank\">".DEFAULT_URL."/sitemgr/".BANNER_FEATURE_FOLDER."/view.php?id=".$banner->getNumber("id")."</a><br /><br />
						</div>
					</body>
				</html>";
			...
			if ($payment_method == "checkout") {
				header("Location: ".DEFAULT_URL."/members/".BANNER_FEATURE_FOLDER."/edit.php?id=".$banner->getNumber("id")."&process=signup");
	...
	$formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/members/login.php?destiny=".EDIRECTORY_FOLDER.(EDIR_LANG_URL ? "/".EDIR_LANGUAGEABBREVIATION : "")."/members/".BANNER_FEATURE_FOLDER."/add.php";
    ...
    <script language="javascript" type="text/javascript">
		...
		function orderCalculate() {
			...
			if (xmlhttp) {
				...
				xmlhttp.open("GET", "<?=DEFAULT_URL;?>/ordercalculateprice.php?item=banner&item_id=<?=$unique_id;?>&type="+get_type+"&expiration_setting="+get_expiration_setting+"&unpaid_impressions="+get_unpaid_impressions+"&discount_id="+get_discount_id, true);
				xmlhttp.send(null);
			}
		}
		...
		function typeSwitch(type, expiration_setting) {
			...
			orderCalculate();
			if (document.formDirectory != undefined)	document.formDirectory.action = "<?=$formloginaction?>&query=type=" + type + "&expiration_setting=" + expiration_setting;
			if (document.formOpenID != undefined)		document.formOpenID.action = "<?=$formloginaction?>&query=type=" + type + "&expiration_setting=" + expiration_setting;
			if (document.formCurrentUser != undefined)	document.formCurrentUser.action = "<?=$formloginaction?>&query=type=" + type + "&expiration_setting=" + expiration_setting;
		}
	...
	<div class="content content-full">
		...
		<div class="content-main">
								...
								<tr>
									<td colspan="2">
										...
										<div id="formFacebookUser" class="<? if ($usertype == "facebookuser") echo "isVisible"; else echo "isHidden"; ?>">
											...
											<? $urlRedirect = "?advertise=yes&advertise_item=banner&item_id=".$unique_id."&destiny=".urlencode(DEFAULT_URL."/".MEMBERS_ALIAS."/".BANNER_FEATURE_FOLDER."/add.php"); ?>
								...
								<tr>
									<td colspan="2">
										<div id="formCurrentUser" class="<? if ($usertype == "currentuser") echo "isVisible"; else echo "isHidden"; ?>">
											<form name="formCurrentUser" method="post" action="<?=$formloginaction;?>&amp;query=type=<?=($_POST["type"]?$_POST["type"]:$_GET["type"])?>&amp;expiration_setting=<?=($_POST["expiration_setting"]?$_POST["expiration_setting"]:$_GET["expiration_setting"])?>">
			...
			<div id="formNewUser" class="<? if ((!$usertype) || ($usertype == "newuser")) echo "isVisible"; else echo "isHidden"; ?>">
				...
				<? if ($message_account || $message_banner || $message_discount || $message_contact) { ?>
					<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardMessage">
						...
						<? if ($message_banner || $message_discount) { ?>
							...
							<? if ($message_banner) { ?>
								<tr>
									<td colspan="2"><p class="errorMessage"><?=$message_banner?></p></td>
								</tr>
							<? } ?>
							<? if ($message_discount) { ?>
								<tr>
				...
				<form name="order_banner" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post" class="standardForm">
					<input type="hidden" name="advertise" value="yes" />
					...
					<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardSIGNUPTable">
						...
						<tr>
							...
							<td><input type="text" name="caption<?=EDIR_DEFAULT_LANGUAGENUMBER;?>" value="<?=${"caption".EDIR_DEFAULT_LANGUAGENUMBER}?>" maxlength="25" class="input-form-banner" /></td>
						</tr>
