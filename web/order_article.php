<?
	...
	if (!in_array($level, $classiLevelValue)) {
		header("Location: ".DEFAULT_URL."/advertise.php?article");
		exit;
	}
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($boolean_seckey && $validate_account && $validate_contact && $validate_article && $validate_discount) {
			...
			if ($emailNotificationObj = system_checkEmail(SYSTEM_ARTICLE_SIGNUP, $contact->getString("lang"))) {
				...
				<html>
					...
					<body>
						<div class=\"email_style_settings\">
							...
							$sitemgr_msg .= "Article :<br /><br />";
							$sitemgr_msg .= "<b>Title: </b>".$article->getString("title")."<br />";
							$sitemgr_msg .= "<br /><a href=\"".DEFAULT_URL."/sitemgr/".ARTICLE_FEATURE_FOLDER."/view.php?id=".$article->getNumber("id")."\" target=\"_blank\">".DEFAULT_URL."/sitemgr/".ARTICLE_FEATURE_FOLDER."/view.php?id=".$article->getNumber("id")."</a><br /><br />
						</div>
					</body>
				</html>";
			...
			if ($sitemgr_article_emails[0]) {
				foreach ($sitemgr_article_emails as $sitemgr_article_email) {
					system_mail($sitemgr_article_email, "[".EDIRECTORY_TITLE."] Signup Notification", $sitemgr_msg, EDIRECTORY_TITLE." <$sitemgr_article_email>", "text/html", '', '', $error);
				}
			}
			}
			////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if ($payment_method == "checkout") {
				if ($hasPackage){
					header("Location: ".DEFAULT_URL."/members/signup/packages.php?item_type=article&item_level=".$level."&checkout=true&payment_method=checkout&item_id=".$article->getNumber("id"));
				} else {
					header("Location: ".DEFAULT_URL."/members/".ARTICLE_FEATURE_FOLDER."/article.php?id=".$article->getNumber("id")."&process=signup");
				}
			} elseif ($payment_method == "invoice") {
				if ($hasPackage){
					header("Location: ".DEFAULT_URL."/members/signup/packages.php?item_type=article&item_level=".$level."&checkout=false&payment_method=invoice&item_id=".$article->getNumber("id"));
				} else {
					header("Location: ".DEFAULT_URL."/members/signup/invoice.php");
				}
			} else {
				if ($hasPackage){
					header("Location: ".DEFAULT_URL."/members/signup/packages.php?item_type=article&item_level=".$level."&checkout=false&payment_method=".$payment_method."&item_id=".$article->getNumber("id"));
				} else {
	...
	$formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/members/login.php?destiny=".EDIRECTORY_FOLDER.(EDIR_LANG_URL ? "/".EDIR_LANGUAGEABBREVIATION : "")."/members/".ARTICLE_FEATURE_FOLDER."/article.php";
    ...
    <script language="javascript" type="text/javascript">
		...
		function orderCalculate() {
			...
			if (xmlhttp) {
				...
				var get_level = 0;
				if (document.order_article.level) get_level = document.order_article.level.value;
				var get_discount_id = "";
				if (document.order_article.discount_id) get_discount_id = document.order_article.discount_id.value;
				xmlhttp.open("GET", "<?=DEFAULT_URL;?>/ordercalculateprice.php?item=article&item_id=<?=$unique_id;?>&level="+get_level+"&discount_id="+get_discount_id, true);
				xmlhttp.send(null);
			}
		}
		...
		function levelSwitch(level) {
			document.order_article.level.value = level;
			orderCalculate();
	...
	<div class="content content-full">
		...
		<div class="content-main">
					...
					<td>
						...
						<table align="center" cellspacing="2" cellpadding="2" border="0" class="standardChooseLevel">
							<? foreach ($levelValue as $value) { ?>
								<tr>
									<th><?=$articleLevelObj->showLevel($value)?></th>
									<td>
										<?
										if ($articleLevelObj->getPrice($value) > 0) {
											echo CURRENCY_SYMBOL.$articleLevelObj->getPrice($value)." ".system_showText(LANG_PER)." ";
											if (payment_getRenewalCycle("article") > 1) {
												echo payment_getRenewalCycle("article")." ";
												echo payment_getRenewalUnitNamePlural("article");
											}else {
								...
								<tr>
									<td colspan="2">
										...
										<div id="formFacebookUser" class="<? if ($usertype == "facebookuser") echo "isVisible"; else echo "isHidden"; ?>">
											...
											<? $urlRedirect = "?advertise=yes&advertise_item=article&item_id=".$unique_id."&destiny=".urlencode(DEFAULT_URL."/".MEMBERS_ALIAS."/".ARTICLE_FEATURE_FOLDER."/article.php"); ?>
			...
			<div id="formNewUser" class="<? if ((!$usertype) || ($usertype == "newuser")) echo "isVisible"; else echo "isHidden"; ?>">
				...
				<? if ($message_account || $message_article || $message_discount || $message_contact) { ?>
					<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardMessage">
						...
						<? } ?>
						<? if ($message_article || $message_discount) { ?>
							<tr>
								<th colspan="2" class="SIGNUPTable-title errorTitle"><?=system_showText(LANG_ARTICLEINFO_ERROR)?></th>
							</tr>
							<? if ($message_article) { ?>
								<tr>
									<td colspan="2"><p class="errorMessage"><?=$message_article?></p></td>
								</tr>
							<? } ?>
							<? if ($message_discount) { ?>
								<tr>
				...
				<form name="order_article" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post" class="standardForm">
					<input type="hidden" name="advertise" value="yes" />
					...
					<table align="center" border="0" cellpadding="2" cellspacing="2" class="standardSIGNUPTable">
						...
						<tr>
							<th colspan="2" class="SIGNUPTable-title"><h2 class="standardSubTitle"><?=system_showText(LANG_ARTICLEINFO)?></h2></th>
						</tr>
						<tr>
							<th>* <?=system_showText(LANG_LABEL_TITLE)?>:</th>
							<td>
								<input type="text" name="title" value="<?=$title?>" class="input-form-article" maxlength="100" onblur="easyFriendlyUrl(this.value, 'friendly_url', '<?=FRIENDLYURL_VALIDCHARS?>', '<?=FRIENDLYURL_SEPARATOR?>');" />
						...
						<? if (PAYMENT_FEATURE == "on") { ?>
							<? if ((CREDITCARDPAYMENT_FEATURE == "on") || (INVOICEPAYMENT_FEATURE == "on")) { ?>
								<tr>
									...
									<td><input type="text" name="discount_id" value="<?=$discount_id?>" class="input-form-article" maxlength="10" onblur="orderCalculate();" /></td>
								</tr>
							<? } ?>
						<? } ?>
					</table>
