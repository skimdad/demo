...
function calendardmy(obj_target) {
...
function cal_prs_date (str_date) {
	...
	if (!RE_NUM.exec(arr_date[0])) return cal_error (showText(LANG_JS_CALENDAR_INVALIDDAYMONTHVALUE) + ": '" + arr_date[0] + "'.\n" + showText(LANG_JS_CALENDAR_ALLOWEDVALUESUNSIGNEDINTEGERS) + ".");
	...
	if (!RE_NUM.exec(arr_date[1])) return cal_error (showText(LANG_JS_CALENDAR_INVALIDMONTHVALUE) + ": '" + arr_date[1] + "'.\n" + showText(LANG_JS_CALENDAR_ALLOWEDVALUESUNSIGNEDINTEGERS) + ".");
	...
	if (arr_date[1] < 1 || arr_date[1] > 12) return cal_error (showText(LANG_JS_CALENDAR_INVALIDMONTHVALUE) + ": '" + arr_date[1] + "'.\n" + showText(LANG_JS_CALENDAR_ALLOWEDRANGE) + " 01-12.");
	...
	if (dt_date.getMonth() != (arr_date[1]-1)) return cal_error (showText(LANG_JS_CALENDAR_INVALIDDAYMONTHVALUE) + ": '" + arr_date[0] + "'.\n" + showText(LANG_JS_CALENDAR_ALLOWEDRANGE) + " 01-"+dt_numdays.getDate()+".");
